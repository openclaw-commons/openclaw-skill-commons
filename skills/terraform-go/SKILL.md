...
```bash
npx clawhub@latest install terraform-go
```
...
```bash
# 使用默认配置
openclaw terraform-go

# 查看帮助
openclaw terraform-go --help

# 指定配置
openclaw terraform-go --config ./config.yaml
```
...
```bash
openclaw terraform-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw terraform-go | openclaw task-decomposer
```
