...
```bash
npx clawhub@latest install sqlite-guide
```
...
```bash
# 使用默认配置
openclaw sqlite-guide

# 查看帮助
openclaw sqlite-guide --help

# 指定配置
openclaw sqlite-guide --config ./config.yaml
```
...
```bash
openclaw sqlite-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sqlite-guide | openclaw task-decomposer
```
