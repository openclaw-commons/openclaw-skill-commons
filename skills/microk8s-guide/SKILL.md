...
```bash
npx clawhub@latest install microk8s-guide
```
...
```bash
# 使用默认配置
openclaw microk8s-guide

# 查看帮助
openclaw microk8s-guide --help

# 指定配置
openclaw microk8s-guide --config ./config.yaml
```
...
```bash
openclaw microk8s-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw microk8s-guide | openclaw task-decomposer
```
