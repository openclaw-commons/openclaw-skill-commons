...
```bash
npx clawhub@latest install research
```
...
# 查看帮助
openclaw research --help

# 指定配置
openclaw research --config ./config.yaml
```
...
```bash
openclaw research --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw research | openclaw task-decomposer
```
