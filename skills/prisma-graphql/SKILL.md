...
```bash
npx clawhub@latest install prisma-graphql
```
...
```bash
# 使用默认配置
openclaw prisma-graphql

# 查看帮助
openclaw prisma-graphql --help

# 指定配置
openclaw prisma-graphql --config ./config.yaml
```
...
```bash
openclaw prisma-graphql --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw prisma-graphql | openclaw task-decomposer
```
