...
```bash
npx clawhub@latest install cac-cli
```
...
# 查看帮助
openclaw cac-cli --help

# 指定配置
openclaw cac-cli --config ./config.yaml
```
...
```bash
openclaw cac-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cac-cli | openclaw task-decomposer
```
