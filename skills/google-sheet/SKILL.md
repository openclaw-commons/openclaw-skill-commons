...
```bash
# 使用默认配置
openclaw google-sheet

# 查看帮助
openclaw google-sheet --help

# 指定配置
openclaw google-sheet --config ./config.yaml
```
...
```bash
openclaw google-sheet --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw google-sheet | openclaw task-decomposer
```
