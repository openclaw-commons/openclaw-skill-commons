...
```bash
npx clawhub@latest install rsa-rust
```
...
# 查看帮助
openclaw rsa-rust --help

# 指定配置
openclaw rsa-rust --config ./config.yaml
```
...
```bash
openclaw rsa-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rsa-rust | openclaw task-decomposer
```
