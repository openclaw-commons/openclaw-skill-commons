...
```bash
npx clawhub@latest install oauth2-go
```
...
# 查看帮助
openclaw oauth2-go --help

# 指定配置
openclaw oauth2-go --config ./config.yaml
```
...
```bash
openclaw oauth2-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw oauth2-go | openclaw task-decomposer
```
