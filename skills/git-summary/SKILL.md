...
```bash
# 使用默认配置
openclaw git-summary

# 查看帮助
openclaw git-summary --help

# 指定配置
openclaw git-summary --config ./config.yaml
```
...
```bash
openclaw git-summary --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw git-summary | openclaw task-decomposer
```
