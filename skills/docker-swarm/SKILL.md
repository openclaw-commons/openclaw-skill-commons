...
```bash
npx clawhub@latest install docker-swarm
```
...
```bash
# 使用默认配置
openclaw docker-swarm

# 查看帮助
openclaw docker-swarm --help

# 指定配置
openclaw docker-swarm --config ./config.yaml
```
...
```bash
openclaw docker-swarm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw docker-swarm | openclaw task-decomposer
```
