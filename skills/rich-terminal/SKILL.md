...
```bash
npx clawhub@latest install rich-terminal
```
...
```bash
# 使用默认配置
openclaw rich-terminal

# 查看帮助
openclaw rich-terminal --help

# 指定配置
openclaw rich-terminal --config ./config.yaml
```
...
```bash
openclaw rich-terminal --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rich-terminal | openclaw task-decomposer
```
