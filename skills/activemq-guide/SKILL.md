...
```bash
npx clawhub@latest install activemq-guide
```
...
```bash
# 使用默认配置
openclaw activemq-guide

# 查看帮助
openclaw activemq-guide --help

# 指定配置
openclaw activemq-guide --config ./config.yaml
```
...
```bash
openclaw activemq-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw activemq-guide | openclaw task-decomposer
```
