...
```bash
npx clawhub@latest install minimist-cli
```
...
```bash
# 使用默认配置
openclaw minimist-cli

# 查看帮助
openclaw minimist-cli --help

# 指定配置
openclaw minimist-cli --config ./config.yaml
```
...
```bash
openclaw minimist-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw minimist-cli | openclaw task-decomposer
```
