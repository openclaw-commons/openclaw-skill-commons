...
```bash
npx clawhub@latest install pulumi-go
```
...
# 查看帮助
openclaw pulumi-go --help

# 指定配置
openclaw pulumi-go --config ./config.yaml
```
...
```bash
openclaw pulumi-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pulumi-go | openclaw task-decomposer
```
