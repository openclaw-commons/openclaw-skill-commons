...
```bash
npx clawhub@latest install fastify-speed
```
...
```bash
# 使用默认配置
openclaw fastify-speed

# 查看帮助
openclaw fastify-speed --help

# 指定配置
openclaw fastify-speed --config ./config.yaml
```
...
```bash
openclaw fastify-speed --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw fastify-speed | openclaw task-decomposer
```
