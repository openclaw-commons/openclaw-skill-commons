...
```bash
npx clawhub@latest install twirp-rpc
```
...
# 查看帮助
openclaw twirp-rpc --help

# 指定配置
openclaw twirp-rpc --config ./config.yaml
```
...
```bash
openclaw twirp-rpc --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw twirp-rpc | openclaw task-decomposer
```
