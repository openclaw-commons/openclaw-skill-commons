...
```bash
npx clawhub@latest install sqlc-queries
```
...
```bash
# 使用默认配置
openclaw sqlc-queries

# 查看帮助
openclaw sqlc-queries --help

# 指定配置
openclaw sqlc-queries --config ./config.yaml
```
...
```bash
openclaw sqlc-queries --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sqlc-queries | openclaw task-decomposer
```
