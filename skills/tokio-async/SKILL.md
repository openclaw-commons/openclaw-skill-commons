...
```bash
npx clawhub@latest install tokio-async
```
...
```bash
# 使用默认配置
openclaw tokio-async

# 查看帮助
openclaw tokio-async --help

# 指定配置
openclaw tokio-async --config ./config.yaml
```
...
```bash
openclaw tokio-async --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tokio-async | openclaw task-decomposer
```
