...
```bash
npx clawhub@latest install fancy-regex
```
...
```bash
# 使用默认配置
openclaw fancy-regex

# 查看帮助
openclaw fancy-regex --help

# 指定配置
openclaw fancy-regex --config ./config.yaml
```
...
```bash
openclaw fancy-regex --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw fancy-regex | openclaw task-decomposer
```
