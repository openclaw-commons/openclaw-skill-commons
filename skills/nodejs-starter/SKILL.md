...
```bash
npx clawhub@latest install nodejs-starter
```
...
```bash
# 使用默认配置
openclaw nodejs-starter

# 查看帮助
openclaw nodejs-starter --help

# 指定配置
openclaw nodejs-starter --config ./config.yaml
```
...
```bash
openclaw nodejs-starter --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nodejs-starter | openclaw task-decomposer
```
