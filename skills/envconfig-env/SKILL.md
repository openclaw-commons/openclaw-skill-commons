...
```bash
npx clawhub@latest install envconfig-env
```
...
```bash
# 使用默认配置
openclaw envconfig-env

# 查看帮助
openclaw envconfig-env --help

# 指定配置
openclaw envconfig-env --config ./config.yaml
```
...
```bash
openclaw envconfig-env --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw envconfig-env | openclaw task-decomposer
```
