...
```bash
npx clawhub@latest install webpack-config
```
...
```bash
# 使用默认配置
openclaw webpack-config

# 查看帮助
openclaw webpack-config --help

# 指定配置
openclaw webpack-config --config ./config.yaml
```
...
```bash
openclaw webpack-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw webpack-config | openclaw task-decomposer
```
