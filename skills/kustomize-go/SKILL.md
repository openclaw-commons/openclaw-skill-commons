...
```bash
npx clawhub@latest install kustomize-go
```
...
```bash
# 使用默认配置
openclaw kustomize-go

# 查看帮助
openclaw kustomize-go --help

# 指定配置
openclaw kustomize-go --config ./config.yaml
```
...
```bash
openclaw kustomize-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw kustomize-go | openclaw task-decomposer
```
