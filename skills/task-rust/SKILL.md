...
```bash
npx clawhub@latest install task-rust
```
...
# 查看帮助
openclaw task-rust --help

# 指定配置
openclaw task-rust --config ./config.yaml
```
...
```bash
openclaw task-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw task-rust | openclaw task-decomposer
```
