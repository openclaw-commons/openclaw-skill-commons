...
```bash
npx clawhub@latest install wasabi-guide
```
...
```bash
# 使用默认配置
openclaw wasabi-guide

# 查看帮助
openclaw wasabi-guide --help

# 指定配置
openclaw wasabi-guide --config ./config.yaml
```
...
```bash
openclaw wasabi-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw wasabi-guide | openclaw task-decomposer
```
