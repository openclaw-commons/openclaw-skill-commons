...
```bash
npx clawhub@latest install cert-manager
```
...
```bash
# 使用默认配置
openclaw cert-manager

# 查看帮助
openclaw cert-manager --help

# 指定配置
openclaw cert-manager --config ./config.yaml
```
...
```bash
openclaw cert-manager --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cert-manager | openclaw task-decomposer
```
