...
```bash
npx clawhub@latest install knex-query
```
...
# 查看帮助
openclaw knex-query --help

# 指定配置
openclaw knex-query --config ./config.yaml
```
...
```bash
openclaw knex-query --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw knex-query | openclaw task-decomposer
```
