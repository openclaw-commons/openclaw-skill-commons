...
```bash
npx clawhub@latest install avro-node
```
...
# 查看帮助
openclaw avro-node --help

# 指定配置
openclaw avro-node --config ./config.yaml
```
...
```bash
openclaw avro-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw avro-node | openclaw task-decomposer
```
