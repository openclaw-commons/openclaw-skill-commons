...
```bash
npx clawhub@latest install jsdom-dom
```
...
# 查看帮助
openclaw jsdom-dom --help

# 指定配置
openclaw jsdom-dom --config ./config.yaml
```
...
```bash
openclaw jsdom-dom --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw jsdom-dom | openclaw task-decomposer
```
