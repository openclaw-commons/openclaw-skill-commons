...
```bash
npx clawhub@latest install osm-guide
```
...
# 查看帮助
openclaw osm-guide --help

# 指定配置
openclaw osm-guide --config ./config.yaml
```
...
```bash
openclaw osm-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw osm-guide | openclaw task-decomposer
```
