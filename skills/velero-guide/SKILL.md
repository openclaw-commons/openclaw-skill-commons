...
```bash
npx clawhub@latest install velero-guide
```
...
```bash
# 使用默认配置
openclaw velero-guide

# 查看帮助
openclaw velero-guide --help

# 指定配置
openclaw velero-guide --config ./config.yaml
```
...
```bash
openclaw velero-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw velero-guide | openclaw task-decomposer
```
