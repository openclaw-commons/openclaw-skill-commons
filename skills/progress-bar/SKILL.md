...
```bash
npx clawhub@latest install progress-bar
```
...
# 查看帮助
openclaw progress-bar --help

# 指定配置
openclaw progress-bar --config ./config.yaml
```
...
```bash
openclaw progress-bar --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw progress-bar | openclaw task-decomposer
```
