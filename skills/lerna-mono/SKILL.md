...
```bash
npx clawhub@latest install lerna-mono
```
...
# 查看帮助
openclaw lerna-mono --help

# 指定配置
openclaw lerna-mono --config ./config.yaml
```
...
```bash
openclaw lerna-mono --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw lerna-mono | openclaw task-decomposer
```
