...
```bash
npx clawhub@latest install json-formatter
```
...
```bash
# 使用默认配置
openclaw json-formatter

# 查看帮助
openclaw json-formatter --help

# 指定配置
openclaw json-formatter --config ./config.yaml
```
...
```bash
openclaw json-formatter --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw json-formatter | openclaw task-decomposer
```
