...
```bash
npx clawhub@latest install clap-cli
```
...
# 查看帮助
openclaw clap-cli --help

# 指定配置
openclaw clap-cli --config ./config.yaml
```
...
```bash
openclaw clap-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw clap-cli | openclaw task-decomposer
```
