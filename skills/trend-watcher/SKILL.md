...
```bash
# 使用默认配置
openclaw trend-watcher

# 查看帮助
openclaw trend-watcher --help

# 指定配置
openclaw trend-watcher --config ./config.yaml
```
...
```bash
openclaw trend-watcher --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw trend-watcher | openclaw task-decomposer
```
