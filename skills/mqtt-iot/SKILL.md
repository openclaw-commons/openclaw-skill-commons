...
```bash
npx clawhub@latest install mqtt-iot
```
...
# 查看帮助
openclaw mqtt-iot --help

# 指定配置
openclaw mqtt-iot --config ./config.yaml
```
...
```bash
openclaw mqtt-iot --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mqtt-iot | openclaw task-decomposer
```
