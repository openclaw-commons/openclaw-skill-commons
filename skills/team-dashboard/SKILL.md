...
```bash
npx clawhub@latest install team-dashboard
```
...
# 查看帮助
openclaw team-dashboard --help

# 指定配置
openclaw team-dashboard --config ./config.yaml
```
...
```bash
openclaw team-dashboard --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw team-dashboard | openclaw task-decomposer
```
