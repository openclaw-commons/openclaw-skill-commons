...
```bash
npx clawhub@latest install eks-guide
```
...
# 查看帮助
openclaw eks-guide --help

# 指定配置
openclaw eks-guide --config ./config.yaml
```
...
```bash
openclaw eks-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw eks-guide | openclaw task-decomposer
```
