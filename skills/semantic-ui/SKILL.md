...
```bash
npx clawhub@latest install semantic-ui
```
...
```bash
# 使用默认配置
openclaw semantic-ui

# 查看帮助
openclaw semantic-ui --help

# 指定配置
openclaw semantic-ui --config ./config.yaml
```
...
```bash
openclaw semantic-ui --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw semantic-ui | openclaw task-decomposer
```
