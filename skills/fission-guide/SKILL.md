...
```bash
npx clawhub@latest install fission-guide
```
...
```bash
# 使用默认配置
openclaw fission-guide

# 查看帮助
openclaw fission-guide --help

# 指定配置
openclaw fission-guide --config ./config.yaml
```
...
```bash
openclaw fission-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw fission-guide | openclaw task-decomposer
```
