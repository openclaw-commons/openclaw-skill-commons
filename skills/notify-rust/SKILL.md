...
```bash
npx clawhub@latest install notify-rust
```
...
```bash
# 使用默认配置
openclaw notify-rust

# 查看帮助
openclaw notify-rust --help

# 指定配置
openclaw notify-rust --config ./config.yaml
```
...
```bash
openclaw notify-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw notify-rust | openclaw task-decomposer
```
