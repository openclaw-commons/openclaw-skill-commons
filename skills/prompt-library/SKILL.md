...
```bash
npx clawhub@latest install prompt-library
```
...
# 查看帮助
openclaw prompt-library --help

# 指定配置
openclaw prompt-library --config ./config.yaml
```
...
```bash
openclaw prompt-library --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw prompt-library | openclaw task-decomposer
```
