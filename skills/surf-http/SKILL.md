...
```bash
npx clawhub@latest install surf-http
```
...
# 查看帮助
openclaw surf-http --help

# 指定配置
openclaw surf-http --config ./config.yaml
```
...
```bash
openclaw surf-http --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw surf-http | openclaw task-decomposer
```
