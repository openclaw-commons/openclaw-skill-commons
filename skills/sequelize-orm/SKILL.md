...
```bash
npx clawhub@latest install sequelize-orm
```
...
```bash
# 使用默认配置
openclaw sequelize-orm

# 查看帮助
openclaw sequelize-orm --help

# 指定配置
openclaw sequelize-orm --config ./config.yaml
```
...
```bash
openclaw sequelize-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sequelize-orm | openclaw task-decomposer
```
