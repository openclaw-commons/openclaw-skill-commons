...
```bash
npx clawhub@latest install whiteboard
```
...
# 查看帮助
openclaw whiteboard --help

# 指定配置
openclaw whiteboard --config ./config.yaml
```
...
```bash
openclaw whiteboard --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw whiteboard | openclaw task-decomposer
```
