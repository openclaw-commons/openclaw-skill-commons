...
```bash
npx clawhub@latest install graphql-api
```
...
```bash
# 使用默认配置
openclaw graphql-api

# 查看帮助
openclaw graphql-api --help

# 指定配置
openclaw graphql-api --config ./config.yaml
```
...
```bash
openclaw graphql-api --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw graphql-api | openclaw task-decomposer
```
