...
```bash
npx clawhub@latest install less-css
```
...
# 查看帮助
openclaw less-css --help

# 指定配置
openclaw less-css --config ./config.yaml
```
...
```bash
openclaw less-css --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw less-css | openclaw task-decomposer
```
