...
```bash
npx clawhub@latest install fastapi-guide
```
...
```bash
# 使用默认配置
openclaw fastapi-guide

# 查看帮助
openclaw fastapi-guide --help

# 指定配置
openclaw fastapi-guide --config ./config.yaml
```
...
```bash
openclaw fastapi-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw fastapi-guide | openclaw task-decomposer
```
