...
```bash
npx clawhub@latest install falcon-api
```
...
# 查看帮助
openclaw falcon-api --help

# 指定配置
openclaw falcon-api --config ./config.yaml
```
...
```bash
openclaw falcon-api --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw falcon-api | openclaw task-decomposer
```
