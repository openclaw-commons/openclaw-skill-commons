...
```bash
npx clawhub@latest install linaria-css
```
...
```bash
# 使用默认配置
openclaw linaria-css

# 查看帮助
openclaw linaria-css --help

# 指定配置
openclaw linaria-css --config ./config.yaml
```
...
```bash
openclaw linaria-css --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw linaria-css | openclaw task-decomposer
```
