...
```bash
npx clawhub@latest install dashmap-rust
```
...
```bash
# 使用默认配置
openclaw dashmap-rust

# 查看帮助
openclaw dashmap-rust --help

# 指定配置
openclaw dashmap-rust --config ./config.yaml
```
...
```bash
openclaw dashmap-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw dashmap-rust | openclaw task-decomposer
```
