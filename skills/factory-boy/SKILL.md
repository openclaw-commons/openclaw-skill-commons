...
```bash
npx clawhub@latest install factory-boy
```
...
```bash
# 使用默认配置
openclaw factory-boy

# 查看帮助
openclaw factory-boy --help

# 指定配置
openclaw factory-boy --config ./config.yaml
```
...
```bash
openclaw factory-boy --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw factory-boy | openclaw task-decomposer
```
