...
```bash
npx clawhub@latest install drone-ci
```
...
# 查看帮助
openclaw drone-ci --help

# 指定配置
openclaw drone-ci --config ./config.yaml
```
...
```bash
openclaw drone-ci --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw drone-ci | openclaw task-decomposer
```
