...
```bash
npx clawhub@latest install couchdb-guide
```
...
```bash
# 使用默认配置
openclaw couchdb-guide

# 查看帮助
openclaw couchdb-guide --help

# 指定配置
openclaw couchdb-guide --config ./config.yaml
```
...
```bash
openclaw couchdb-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw couchdb-guide | openclaw task-decomposer
```
