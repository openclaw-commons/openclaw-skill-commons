...
```bash
npx clawhub@latest install ionic-guide
```
...
```bash
# 使用默认配置
openclaw ionic-guide

# 查看帮助
openclaw ionic-guide --help

# 指定配置
openclaw ionic-guide --config ./config.yaml
```
...
```bash
openclaw ionic-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ionic-guide | openclaw task-decomposer
```
