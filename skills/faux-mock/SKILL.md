...
```bash
npx clawhub@latest install faux-mock
```
...
# 查看帮助
openclaw faux-mock --help

# 指定配置
openclaw faux-mock --config ./config.yaml
```
...
```bash
openclaw faux-mock --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw faux-mock | openclaw task-decomposer
```
