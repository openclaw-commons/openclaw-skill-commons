...
```bash
npx clawhub@latest install pomodoro-timer
```
...
```bash
# 使用默认配置
openclaw pomodoro-timer

# 查看帮助
openclaw pomodoro-timer --help

# 指定配置
openclaw pomodoro-timer --config ./config.yaml
```
...
```bash
openclaw pomodoro-timer --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pomodoro-timer | openclaw task-decomposer
```
