...
```bash
npx clawhub@latest install dig-di
```
...
# 查看帮助
openclaw dig-di --help

# 指定配置
openclaw dig-di --config ./config.yaml
```
...
```bash
openclaw dig-di --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw dig-di | openclaw task-decomposer
```
