...
```bash
npx clawhub@latest install csv-parser
```
...
# 查看帮助
openclaw csv-parser --help

# 指定配置
openclaw csv-parser --config ./config.yaml
```
...
```bash
openclaw csv-parser --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw csv-parser | openclaw task-decomposer
```
