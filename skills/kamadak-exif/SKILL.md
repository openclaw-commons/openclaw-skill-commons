...
```bash
npx clawhub@latest install kamadak-exif
```
...
```bash
# 使用默认配置
openclaw kamadak-exif

# 查看帮助
openclaw kamadak-exif --help

# 指定配置
openclaw kamadak-exif --config ./config.yaml
```
...
```bash
openclaw kamadak-exif --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw kamadak-exif | openclaw task-decomposer
```
