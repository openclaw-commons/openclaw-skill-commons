...
```bash
npx clawhub@latest install aes-rust
```
...
# 查看帮助
openclaw aes-rust --help

# 指定配置
openclaw aes-rust --config ./config.yaml
```
...
```bash
openclaw aes-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw aes-rust | openclaw task-decomposer
```
