...
```bash
npx clawhub@latest install cv-opencv
```
...
# 查看帮助
openclaw cv-opencv --help

# 指定配置
openclaw cv-opencv --config ./config.yaml
```
...
```bash
openclaw cv-opencv --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cv-opencv | openclaw task-decomposer
```
