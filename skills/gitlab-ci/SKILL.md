...
```bash
npx clawhub@latest install gitlab-ci
```
...
# 查看帮助
openclaw gitlab-ci --help

# 指定配置
openclaw gitlab-ci --config ./config.yaml
```
...
```bash
openclaw gitlab-ci --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gitlab-ci | openclaw task-decomposer
```
