...
```bash
npx clawhub@latest install unpkg-cdn
```
...
# 查看帮助
openclaw unpkg-cdn --help

# 指定配置
openclaw unpkg-cdn --config ./config.yaml
```
...
```bash
openclaw unpkg-cdn --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw unpkg-cdn | openclaw task-decomposer
```
