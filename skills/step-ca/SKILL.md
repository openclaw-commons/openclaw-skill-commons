...
```bash
npx clawhub@latest install step-ca
```
...
# 查看帮助
openclaw step-ca --help

# 指定配置
openclaw step-ca --config ./config.yaml
```
...
```bash
openclaw step-ca --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw step-ca | openclaw task-decomposer
```
