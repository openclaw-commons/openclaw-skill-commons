...
```bash
npx clawhub@latest install podman-go
```
...
# 查看帮助
openclaw podman-go --help

# 指定配置
openclaw podman-go --config ./config.yaml
```
...
```bash
openclaw podman-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw podman-go | openclaw task-decomposer
```
