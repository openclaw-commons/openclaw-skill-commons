...
```bash
npx clawhub@latest install k3s-guide
```
...
# 查看帮助
openclaw k3s-guide --help

# 指定配置
openclaw k3s-guide --config ./config.yaml
```
...
```bash
openclaw k3s-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw k3s-guide | openclaw task-decomposer
```
