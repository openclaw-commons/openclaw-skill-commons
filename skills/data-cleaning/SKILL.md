...
```bash
npx clawhub@latest install data-cleaning
```
...
```bash
# 使用默认配置
openclaw data-cleaning

# 查看帮助
openclaw data-cleaning --help

# 指定配置
openclaw data-cleaning --config ./config.yaml
```
...
```bash
openclaw data-cleaning --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw data-cleaning | openclaw task-decomposer
```
