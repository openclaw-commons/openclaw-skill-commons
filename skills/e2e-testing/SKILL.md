...
```bash
npx clawhub@latest install e2e-testing
```
...
```bash
# 使用默认配置
openclaw e2e-testing

# 查看帮助
openclaw e2e-testing --help

# 指定配置
openclaw e2e-testing --config ./config.yaml
```
...
```bash
openclaw e2e-testing --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw e2e-testing | openclaw task-decomposer
```
