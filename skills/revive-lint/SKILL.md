...
```bash
npx clawhub@latest install revive-lint
```
...
```bash
# 使用默认配置
openclaw revive-lint

# 查看帮助
openclaw revive-lint --help

# 指定配置
openclaw revive-lint --config ./config.yaml
```
...
```bash
openclaw revive-lint --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw revive-lint | openclaw task-decomposer
```
