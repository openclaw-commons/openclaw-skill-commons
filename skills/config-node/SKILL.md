...
```bash
npx clawhub@latest install config-node
```
...
# 查看帮助
openclaw config-node --help

# 指定配置
openclaw config-node --config ./config.yaml
```
...
```bash
openclaw config-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw config-node | openclaw task-decomposer
```
