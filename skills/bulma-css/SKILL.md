...
```bash
npx clawhub@latest install bulma-css
```
...
# 查看帮助
openclaw bulma-css --help

# 指定配置
openclaw bulma-css --config ./config.yaml
```
...
```bash
openclaw bulma-css --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bulma-css | openclaw task-decomposer
```
