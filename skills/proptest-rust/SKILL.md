...
```bash
npx clawhub@latest install proptest-rust
```
...
```bash
# 使用默认配置
openclaw proptest-rust

# 查看帮助
openclaw proptest-rust --help

# 指定配置
openclaw proptest-rust --config ./config.yaml
```
...
```bash
openclaw proptest-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw proptest-rust | openclaw task-decomposer
```
