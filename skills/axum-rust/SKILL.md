...
```bash
npx clawhub@latest install axum-rust
```
...
# 查看帮助
openclaw axum-rust --help

# 指定配置
openclaw axum-rust --config ./config.yaml
```
...
```bash
openclaw axum-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw axum-rust | openclaw task-decomposer
```
