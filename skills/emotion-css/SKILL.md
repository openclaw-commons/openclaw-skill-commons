...
```bash
npx clawhub@latest install emotion-css
```
...
```bash
# 使用默认配置
openclaw emotion-css

# 查看帮助
openclaw emotion-css --help

# 指定配置
openclaw emotion-css --config ./config.yaml
```
...
```bash
openclaw emotion-css --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw emotion-css | openclaw task-decomposer
```
