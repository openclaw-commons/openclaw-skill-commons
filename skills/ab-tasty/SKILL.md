...
```bash
npx clawhub@latest install ab-tasty
```
...
# 查看帮助
openclaw ab-tasty --help

# 指定配置
openclaw ab-tasty --config ./config.yaml
```
...
```bash
openclaw ab-tasty --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ab-tasty | openclaw task-decomposer
```
