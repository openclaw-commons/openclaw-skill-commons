...
```bash
npx clawhub@latest install nom-parser
```
...
# 查看帮助
openclaw nom-parser --help

# 指定配置
openclaw nom-parser --config ./config.yaml
```
...
```bash
openclaw nom-parser --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nom-parser | openclaw task-decomposer
```
