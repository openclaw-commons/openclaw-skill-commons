...
```bash
npx clawhub@latest install arq-guide
```
...
# 查看帮助
openclaw arq-guide --help

# 指定配置
openclaw arq-guide --config ./config.yaml
```
...
```bash
openclaw arq-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw arq-guide | openclaw task-decomposer
```
