...
```bash
npx clawhub@latest install seaborn-viz
```
...
```bash
# 使用默认配置
openclaw seaborn-viz

# 查看帮助
openclaw seaborn-viz --help

# 指定配置
openclaw seaborn-viz --config ./config.yaml
```
...
```bash
openclaw seaborn-viz --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw seaborn-viz | openclaw task-decomposer
```
