...
```bash
npx clawhub@latest install egui-gui
```
...
# 查看帮助
openclaw egui-gui --help

# 指定配置
openclaw egui-gui --config ./config.yaml
```
...
```bash
openclaw egui-gui --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw egui-gui | openclaw task-decomposer
```
