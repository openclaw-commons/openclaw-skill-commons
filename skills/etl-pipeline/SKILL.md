...
```bash
npx clawhub@latest install etl-pipeline
```
...
```bash
# 使用默认配置
openclaw etl-pipeline

# 查看帮助
openclaw etl-pipeline --help

# 指定配置
openclaw etl-pipeline --config ./config.yaml
```
...
```bash
openclaw etl-pipeline --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw etl-pipeline | openclaw task-decomposer
```
