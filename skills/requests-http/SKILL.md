...
```bash
npx clawhub@latest install requests-http
```
...
```bash
# 使用默认配置
openclaw requests-http

# 查看帮助
openclaw requests-http --help

# 指定配置
openclaw requests-http --config ./config.yaml
```
...
```bash
openclaw requests-http --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw requests-http | openclaw task-decomposer
```
