...
```bash
npx clawhub@latest install gcp-setup
```
...
# 查看帮助
openclaw gcp-setup --help

# 指定配置
openclaw gcp-setup --config ./config.yaml
```
...
```bash
openclaw gcp-setup --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gcp-setup | openclaw task-decomposer
```
