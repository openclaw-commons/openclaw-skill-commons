...
```bash
npx clawhub@latest install datadog-guide
```
...
```bash
# 使用默认配置
openclaw datadog-guide

# 查看帮助
openclaw datadog-guide --help

# 指定配置
openclaw datadog-guide --config ./config.yaml
```
...
```bash
openclaw datadog-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw datadog-guide | openclaw task-decomposer
```
