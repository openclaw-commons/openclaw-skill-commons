...
```bash
npx clawhub@latest install golangci-lint
```
...
```bash
# 使用默认配置
openclaw golangci-lint

# 查看帮助
openclaw golangci-lint --help

# 指定配置
openclaw golangci-lint --config ./config.yaml
```
...
```bash
openclaw golangci-lint --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw golangci-lint | openclaw task-decomposer
```
