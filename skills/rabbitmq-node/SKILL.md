...
```bash
npx clawhub@latest install rabbitmq-node
```
...
```bash
# 使用默认配置
openclaw rabbitmq-node

# 查看帮助
openclaw rabbitmq-node --help

# 指定配置
openclaw rabbitmq-node --config ./config.yaml
```
...
```bash
openclaw rabbitmq-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rabbitmq-node | openclaw task-decomposer
```
