...
```bash
npx clawhub@latest install bottle-micro
```
...
```bash
# 使用默认配置
openclaw bottle-micro

# 查看帮助
openclaw bottle-micro --help

# 指定配置
openclaw bottle-micro --config ./config.yaml
```
...
```bash
openclaw bottle-micro --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bottle-micro | openclaw task-decomposer
```
