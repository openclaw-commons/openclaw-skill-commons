...
```bash
npx clawhub@latest install istio-go
```
...
# 查看帮助
openclaw istio-go --help

# 指定配置
openclaw istio-go --config ./config.yaml
```
...
```bash
openclaw istio-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw istio-go | openclaw task-decomposer
```
