...
```bash
npx clawhub@latest install boxen-terminal
```
...
```bash
# 使用默认配置
openclaw boxen-terminal

# 查看帮助
openclaw boxen-terminal --help

# 指定配置
openclaw boxen-terminal --config ./config.yaml
```
...
```bash
openclaw boxen-terminal --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw boxen-terminal | openclaw task-decomposer
```
