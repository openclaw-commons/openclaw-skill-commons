...
```bash
npx clawhub@latest install oauth2-guide
```
...
```bash
# 使用默认配置
openclaw oauth2-guide

# 查看帮助
openclaw oauth2-guide --help

# 指定配置
openclaw oauth2-guide --config ./config.yaml
```
...
```bash
openclaw oauth2-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw oauth2-guide | openclaw task-decomposer
```
