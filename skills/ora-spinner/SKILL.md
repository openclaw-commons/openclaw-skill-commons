...
```bash
npx clawhub@latest install ora-spinner
```
...
```bash
# 使用默认配置
openclaw ora-spinner

# 查看帮助
openclaw ora-spinner --help

# 指定配置
openclaw ora-spinner --config ./config.yaml
```
...
```bash
openclaw ora-spinner --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ora-spinner | openclaw task-decomposer
```
