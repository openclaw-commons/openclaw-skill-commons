...
```bash
npx clawhub@latest install vue-starter
```
...
```bash
# 使用默认配置
openclaw vue-starter

# 查看帮助
openclaw vue-starter --help

# 指定配置
openclaw vue-starter --config ./config.yaml
```
...
```bash
openclaw vue-starter --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw vue-starter | openclaw task-decomposer
```
