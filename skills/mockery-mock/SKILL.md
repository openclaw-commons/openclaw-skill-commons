...
```bash
npx clawhub@latest install mockery-mock
```
...
```bash
# 使用默认配置
openclaw mockery-mock

# 查看帮助
openclaw mockery-mock --help

# 指定配置
openclaw mockery-mock --config ./config.yaml
```
...
```bash
openclaw mockery-mock --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mockery-mock | openclaw task-decomposer
```
