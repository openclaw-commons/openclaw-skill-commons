...
```bash
npx clawhub@latest install sqlalchemy-orm
```
...
```bash
# 使用默认配置
openclaw sqlalchemy-orm

# 查看帮助
openclaw sqlalchemy-orm --help

# 指定配置
openclaw sqlalchemy-orm --config ./config.yaml
```
...
```bash
openclaw sqlalchemy-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sqlalchemy-orm | openclaw task-decomposer
```
