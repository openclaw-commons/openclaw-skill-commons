...
```bash
npx clawhub@latest install gotest-tools
```
...
```bash
# 使用默认配置
openclaw gotest-tools

# 查看帮助
openclaw gotest-tools --help

# 指定配置
openclaw gotest-tools --config ./config.yaml
```
...
```bash
openclaw gotest-tools --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gotest-tools | openclaw task-decomposer
```
