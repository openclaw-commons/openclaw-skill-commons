...
```bash
npx clawhub@latest install prisma-orm
```
...
# 查看帮助
openclaw prisma-orm --help

# 指定配置
openclaw prisma-orm --config ./config.yaml
```
...
```bash
openclaw prisma-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw prisma-orm | openclaw task-decomposer
```
