...
```bash
npx clawhub@latest install immunio-guide
```
...
```bash
# 使用默认配置
openclaw immunio-guide

# 查看帮助
openclaw immunio-guide --help

# 指定配置
openclaw immunio-guide --config ./config.yaml
```
...
```bash
openclaw immunio-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw immunio-guide | openclaw task-decomposer
```
