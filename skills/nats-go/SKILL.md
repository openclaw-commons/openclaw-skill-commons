...
```bash
npx clawhub@latest install nats-go
```
...
# 查看帮助
openclaw nats-go --help

# 指定配置
openclaw nats-go --config ./config.yaml
```
...
```bash
openclaw nats-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nats-go | openclaw task-decomposer
```
