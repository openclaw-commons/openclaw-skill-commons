...
```bash
# 使用默认配置
openclaw network-scan

# 查看帮助
openclaw network-scan --help

# 指定配置
openclaw network-scan --config ./config.yaml
```
...
```bash
openclaw network-scan --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw network-scan | openclaw task-decomposer
```
