...
```bash
npx clawhub@latest install kingpin-cli
```
...
```bash
# 使用默认配置
openclaw kingpin-cli

# 查看帮助
openclaw kingpin-cli --help

# 指定配置
openclaw kingpin-cli --config ./config.yaml
```
...
```bash
openclaw kingpin-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw kingpin-cli | openclaw task-decomposer
```
