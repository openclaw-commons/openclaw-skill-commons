...
```bash
npx clawhub@latest install vite-build
```
...
# 查看帮助
openclaw vite-build --help

# 指定配置
openclaw vite-build --config ./config.yaml
```
...
```bash
openclaw vite-build --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw vite-build | openclaw task-decomposer
```
