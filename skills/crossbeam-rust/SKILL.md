...
```bash
npx clawhub@latest install crossbeam-rust
```
...
```bash
# 使用默认配置
openclaw crossbeam-rust

# 查看帮助
openclaw crossbeam-rust --help

# 指定配置
openclaw crossbeam-rust --config ./config.yaml
```
...
```bash
openclaw crossbeam-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw crossbeam-rust | openclaw task-decomposer
```
