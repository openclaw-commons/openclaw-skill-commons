...
```bash
npx clawhub@latest install gatsby-static
```
...
```bash
# 使用默认配置
openclaw gatsby-static

# 查看帮助
openclaw gatsby-static --help

# 指定配置
openclaw gatsby-static --config ./config.yaml
```
...
```bash
openclaw gatsby-static --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gatsby-static | openclaw task-decomposer
```
