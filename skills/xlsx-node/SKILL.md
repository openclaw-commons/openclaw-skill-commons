...
```bash
npx clawhub@latest install xlsx-node
```
...
# 查看帮助
openclaw xlsx-node --help

# 指定配置
openclaw xlsx-node --config ./config.yaml
```
...
```bash
openclaw xlsx-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw xlsx-node | openclaw task-decomposer
```
