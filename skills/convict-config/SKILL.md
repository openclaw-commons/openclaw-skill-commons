...
```bash
npx clawhub@latest install convict-config
```
...
```bash
# 使用默认配置
openclaw convict-config

# 查看帮助
openclaw convict-config --help

# 指定配置
openclaw convict-config --config ./config.yaml
```
...
```bash
openclaw convict-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw convict-config | openclaw task-decomposer
```
