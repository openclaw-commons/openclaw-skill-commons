...
```bash
npx clawhub@latest install celery-go
```
...
# 查看帮助
openclaw celery-go --help

# 指定配置
openclaw celery-go --config ./config.yaml
```
...
```bash
openclaw celery-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw celery-go | openclaw task-decomposer
```
