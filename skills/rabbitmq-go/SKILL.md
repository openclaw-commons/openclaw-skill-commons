...
```bash
npx clawhub@latest install rabbitmq-go
```
...
```bash
# 使用默认配置
openclaw rabbitmq-go

# 查看帮助
openclaw rabbitmq-go --help

# 指定配置
openclaw rabbitmq-go --config ./config.yaml
```
...
```bash
openclaw rabbitmq-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rabbitmq-go | openclaw task-decomposer
```
