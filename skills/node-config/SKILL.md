...
```bash
npx clawhub@latest install node-config
```
...
# 查看帮助
openclaw node-config --help

# 指定配置
openclaw node-config --config ./config.yaml
```
...
```bash
openclaw node-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw node-config | openclaw task-decomposer
```
