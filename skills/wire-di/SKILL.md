...
```bash
npx clawhub@latest install wire-di
```
...
# 查看帮助
openclaw wire-di --help

# 指定配置
openclaw wire-di --config ./config.yaml
```
...
```bash
openclaw wire-di --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw wire-di | openclaw task-decomposer
```
