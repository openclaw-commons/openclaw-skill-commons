...
```bash
npx clawhub@latest install palette-color
```
...
```bash
# 使用默认配置
openclaw palette-color

# 查看帮助
openclaw palette-color --help

# 指定配置
openclaw palette-color --config ./config.yaml
```
...
```bash
openclaw palette-color --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw palette-color | openclaw task-decomposer
```
