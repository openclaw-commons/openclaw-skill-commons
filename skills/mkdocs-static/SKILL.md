...
```bash
npx clawhub@latest install mkdocs-static
```
...
```bash
# 使用默认配置
openclaw mkdocs-static

# 查看帮助
openclaw mkdocs-static --help

# 指定配置
openclaw mkdocs-static --config ./config.yaml
```
...
```bash
openclaw mkdocs-static --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mkdocs-static | openclaw task-decomposer
```
