...
```bash
npx clawhub@latest install pm2-process
```
...
```bash
# 使用默认配置
openclaw pm2-process

# 查看帮助
openclaw pm2-process --help

# 指定配置
openclaw pm2-process --config ./config.yaml
```
...
```bash
openclaw pm2-process --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pm2-process | openclaw task-decomposer
```
