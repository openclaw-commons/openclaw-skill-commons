...
```bash
npx clawhub@latest install csv-parse
```
...
# 查看帮助
openclaw csv-parse --help

# 指定配置
openclaw csv-parse --config ./config.yaml
```
...
```bash
openclaw csv-parse --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw csv-parse | openclaw task-decomposer
```
