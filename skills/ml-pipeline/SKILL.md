...
```bash
npx clawhub@latest install ml-pipeline
```
...
```bash
# 使用默认配置
openclaw ml-pipeline

# 查看帮助
openclaw ml-pipeline --help

# 指定配置
openclaw ml-pipeline --config ./config.yaml
```
...
```bash
openclaw ml-pipeline --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ml-pipeline | openclaw task-decomposer
```
