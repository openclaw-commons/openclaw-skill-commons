...
```bash
npx clawhub@latest install combine-parser
```
...
```bash
# 使用默认配置
openclaw combine-parser

# 查看帮助
openclaw combine-parser --help

# 指定配置
openclaw combine-parser --config ./config.yaml
```
...
```bash
openclaw combine-parser --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw combine-parser | openclaw task-decomposer
```
