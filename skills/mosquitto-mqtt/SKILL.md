...
```bash
npx clawhub@latest install mosquitto-mqtt
```
...
```bash
# 使用默认配置
openclaw mosquitto-mqtt

# 查看帮助
openclaw mosquitto-mqtt --help

# 指定配置
openclaw mosquitto-mqtt --config ./config.yaml
```
...
```bash
openclaw mosquitto-mqtt --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mosquitto-mqtt | openclaw task-decomposer
```
