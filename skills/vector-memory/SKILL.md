...
```bash
npx clawhub@latest install vector-memory
```
...
# 查看帮助
openclaw vector-memory --help

# 指定配置
openclaw vector-memory --config ./config.yaml
```
...
```bash
openclaw vector-memory --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw vector-memory | openclaw task-decomposer
```
