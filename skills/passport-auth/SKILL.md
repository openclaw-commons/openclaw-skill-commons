...
```bash
npx clawhub@latest install passport-auth
```
...
```bash
# 使用默认配置
openclaw passport-auth

# 查看帮助
openclaw passport-auth --help

# 指定配置
openclaw passport-auth --config ./config.yaml
```
...
```bash
openclaw passport-auth --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw passport-auth | openclaw task-decomposer
```
