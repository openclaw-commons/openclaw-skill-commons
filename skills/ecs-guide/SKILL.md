...
```bash
npx clawhub@latest install ecs-guide
```
...
# 查看帮助
openclaw ecs-guide --help

# 指定配置
openclaw ecs-guide --config ./config.yaml
```
...
```bash
openclaw ecs-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ecs-guide | openclaw task-decomposer
```
