...
```bash
npx clawhub@latest install termion-rust
```
...
```bash
# 使用默认配置
openclaw termion-rust

# 查看帮助
openclaw termion-rust --help

# 指定配置
openclaw termion-rust --config ./config.yaml
```
...
```bash
openclaw termion-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw termion-rust | openclaw task-decomposer
```
