...
```bash
# 使用默认配置
openclaw arxiv-watcher

# 查看帮助
openclaw arxiv-watcher --help

# 指定配置
openclaw arxiv-watcher --config ./config.yaml
```
...
```bash
openclaw arxiv-watcher --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw arxiv-watcher | openclaw task-decomposer
```
