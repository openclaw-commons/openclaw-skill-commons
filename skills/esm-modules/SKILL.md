...
```bash
npx clawhub@latest install esm-modules
```
...
```bash
# 使用默认配置
openclaw esm-modules

# 查看帮助
openclaw esm-modules --help

# 指定配置
openclaw esm-modules --config ./config.yaml
```
...
```bash
openclaw esm-modules --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw esm-modules | openclaw task-decomposer
```
