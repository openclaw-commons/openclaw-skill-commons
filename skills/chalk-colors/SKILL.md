...
```bash
npx clawhub@latest install chalk-colors
```
...
```bash
# 使用默认配置
openclaw chalk-colors

# 查看帮助
openclaw chalk-colors --help

# 指定配置
openclaw chalk-colors --config ./config.yaml
```
...
```bash
openclaw chalk-colors --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw chalk-colors | openclaw task-decomposer
```
