...
```bash
npx clawhub@latest install got-http
```
...
# 查看帮助
openclaw got-http --help

# 指定配置
openclaw got-http --config ./config.yaml
```
...
```bash
openclaw got-http --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw got-http | openclaw task-decomposer
```
