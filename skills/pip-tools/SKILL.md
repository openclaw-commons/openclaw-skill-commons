...
```bash
npx clawhub@latest install pip-tools
```
...
# 查看帮助
openclaw pip-tools --help

# 指定配置
openclaw pip-tools --config ./config.yaml
```
...
```bash
openclaw pip-tools --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pip-tools | openclaw task-decomposer
```
