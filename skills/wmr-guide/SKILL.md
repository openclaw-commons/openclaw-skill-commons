...
```bash
npx clawhub@latest install wmr-guide
```
...
# 查看帮助
openclaw wmr-guide --help

# 指定配置
openclaw wmr-guide --config ./config.yaml
```
...
```bash
openclaw wmr-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw wmr-guide | openclaw task-decomposer
```
