...
```bash
npx clawhub@latest install sqlx-rust
```
...
# 查看帮助
openclaw sqlx-rust --help

# 指定配置
openclaw sqlx-rust --config ./config.yaml
```
...
```bash
openclaw sqlx-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sqlx-rust | openclaw task-decomposer
```
