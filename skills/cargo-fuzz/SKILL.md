...
```bash
npx clawhub@latest install cargo-fuzz
```
...
# 查看帮助
openclaw cargo-fuzz --help

# 指定配置
openclaw cargo-fuzz --config ./config.yaml
```
...
```bash
openclaw cargo-fuzz --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cargo-fuzz | openclaw task-decomposer
```
