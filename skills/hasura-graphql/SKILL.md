...
```bash
npx clawhub@latest install hasura-graphql
```
...
```bash
# 使用默认配置
openclaw hasura-graphql

# 查看帮助
openclaw hasura-graphql --help

# 指定配置
openclaw hasura-graphql --config ./config.yaml
```
...
```bash
openclaw hasura-graphql --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw hasura-graphql | openclaw task-decomposer
```
