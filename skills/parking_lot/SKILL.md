...
```bash
npx clawhub@latest install parking_lot
```
...
```bash
# 使用默认配置
openclaw parking_lot

# 查看帮助
openclaw parking_lot --help

# 指定配置
openclaw parking_lot --config ./config.yaml
```
...
```bash
openclaw parking_lot --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw parking_lot | openclaw task-decomposer
```
