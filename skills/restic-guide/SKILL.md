...
```bash
npx clawhub@latest install restic-guide
```
...
```bash
# 使用默认配置
openclaw restic-guide

# 查看帮助
openclaw restic-guide --help

# 指定配置
openclaw restic-guide --config ./config.yaml
```
...
```bash
openclaw restic-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw restic-guide | openclaw task-decomposer
```
