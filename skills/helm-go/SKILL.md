...
```bash
npx clawhub@latest install helm-go
```
...
# 查看帮助
openclaw helm-go --help

# 指定配置
openclaw helm-go --config ./config.yaml
```
...
```bash
openclaw helm-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw helm-go | openclaw task-decomposer
```
