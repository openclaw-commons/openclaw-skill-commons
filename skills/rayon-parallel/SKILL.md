...
```bash
npx clawhub@latest install rayon-parallel
```
...
```bash
# 使用默认配置
openclaw rayon-parallel

# 查看帮助
openclaw rayon-parallel --help

# 指定配置
openclaw rayon-parallel --config ./config.yaml
```
...
```bash
openclaw rayon-parallel --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rayon-parallel | openclaw task-decomposer
```
