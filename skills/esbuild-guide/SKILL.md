...
```bash
npx clawhub@latest install esbuild-guide
```
...
```bash
# 使用默认配置
openclaw esbuild-guide

# 查看帮助
openclaw esbuild-guide --help

# 指定配置
openclaw esbuild-guide --config ./config.yaml
```
...
```bash
openclaw esbuild-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw esbuild-guide | openclaw task-decomposer
```
