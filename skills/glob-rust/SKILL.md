...
```bash
npx clawhub@latest install glob-rust
```
...
# 查看帮助
openclaw glob-rust --help

# 指定配置
openclaw glob-rust --config ./config.yaml
```
...
```bash
openclaw glob-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw glob-rust | openclaw task-decomposer
```
