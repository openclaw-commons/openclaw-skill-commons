...
```bash
npx clawhub@latest install gron-scheduler
```
...
```bash
# 使用默认配置
openclaw gron-scheduler

# 查看帮助
openclaw gron-scheduler --help

# 指定配置
openclaw gron-scheduler --config ./config.yaml
```
...
```bash
openclaw gron-scheduler --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gron-scheduler | openclaw task-decomposer
```
