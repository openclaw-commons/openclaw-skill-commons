...
```bash
npx clawhub@latest install pytorch-deep
```
...
```bash
# 使用默认配置
openclaw pytorch-deep

# 查看帮助
openclaw pytorch-deep --help

# 指定配置
openclaw pytorch-deep --config ./config.yaml
```
...
```bash
openclaw pytorch-deep --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pytorch-deep | openclaw task-decomposer
```
