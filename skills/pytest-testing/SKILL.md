...
```bash
npx clawhub@latest install pytest-testing
```
...
```bash
# 使用默认配置
openclaw pytest-testing

# 查看帮助
openclaw pytest-testing --help

# 指定配置
openclaw pytest-testing --config ./config.yaml
```
...
```bash
openclaw pytest-testing --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pytest-testing | openclaw task-decomposer
```
