...
```bash
npx clawhub@latest install bash-scripting
```
...
```bash
# 使用默认配置
openclaw bash-scripting

# 查看帮助
openclaw bash-scripting --help

# 指定配置
openclaw bash-scripting --config ./config.yaml
```
...
```bash
openclaw bash-scripting --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bash-scripting | openclaw task-decomposer
```
