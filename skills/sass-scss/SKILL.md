...
```bash
npx clawhub@latest install sass-scss
```
...
# 查看帮助
openclaw sass-scss --help

# 指定配置
openclaw sass-scss --config ./config.yaml
```
...
```bash
openclaw sass-scss --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sass-scss | openclaw task-decomposer
```
