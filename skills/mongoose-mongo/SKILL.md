...
```bash
npx clawhub@latest install mongoose-mongo
```
...
```bash
# 使用默认配置
openclaw mongoose-mongo

# 查看帮助
openclaw mongoose-mongo --help

# 指定配置
openclaw mongoose-mongo --config ./config.yaml
```
...
```bash
openclaw mongoose-mongo --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mongoose-mongo | openclaw task-decomposer
```
