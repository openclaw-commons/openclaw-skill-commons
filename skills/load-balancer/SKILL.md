...
```bash
# 使用默认配置
openclaw load-balancer

# 查看帮助
openclaw load-balancer --help

# 指定配置
openclaw load-balancer --config ./config.yaml
```
...
```bash
openclaw load-balancer --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw load-balancer | openclaw task-decomposer
```
