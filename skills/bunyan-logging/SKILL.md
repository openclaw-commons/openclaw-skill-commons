...
```bash
npx clawhub@latest install bunyan-logging
```
...
```bash
# 使用默认配置
openclaw bunyan-logging

# 查看帮助
openclaw bunyan-logging --help

# 指定配置
openclaw bunyan-logging --config ./config.yaml
```
...
```bash
openclaw bunyan-logging --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bunyan-logging | openclaw task-decomposer
```
