...
```bash
npx clawhub@latest install consul-connect
```
...
```bash
# 使用默认配置
openclaw consul-connect

# 查看帮助
openclaw consul-connect --help

# 指定配置
openclaw consul-connect --config ./config.yaml
```
...
```bash
openclaw consul-connect --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw consul-connect | openclaw task-decomposer
```
