...
```bash
npx clawhub@latest install diagram-tool
```
...
```bash
# 使用默认配置
openclaw diagram-tool

# 查看帮助
openclaw diagram-tool --help

# 指定配置
openclaw diagram-tool --config ./config.yaml
```
...
```bash
openclaw diagram-tool --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw diagram-tool | openclaw task-decomposer
```
