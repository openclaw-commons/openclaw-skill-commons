...
```bash
npx clawhub@latest install email-filters
```
...
```bash
# 使用默认配置
openclaw email-filters

# 查看帮助
openclaw email-filters --help

# 指定配置
openclaw email-filters --config ./config.yaml
```
...
```bash
openclaw email-filters --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw email-filters | openclaw task-decomposer
```
