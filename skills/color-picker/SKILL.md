...
```bash
npx clawhub@latest install color-picker
```
...
```bash
# 使用默认配置
openclaw color-picker

# 查看帮助
openclaw color-picker --help

# 指定配置
openclaw color-picker --config ./config.yaml
```
...
```bash
openclaw color-picker --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw color-picker | openclaw task-decomposer
```
