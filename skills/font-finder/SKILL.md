...
```bash
npx clawhub@latest install font-finder
```
...
```bash
# 使用默认配置
openclaw font-finder

# 查看帮助
openclaw font-finder --help

# 指定配置
openclaw font-finder --config ./config.yaml
```
...
```bash
openclaw font-finder --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw font-finder | openclaw task-decomposer
```
