...
```bash
npx clawhub@latest install protobuf-go
```
...
```bash
# 使用默认配置
openclaw protobuf-go

# 查看帮助
openclaw protobuf-go --help

# 指定配置
openclaw protobuf-go --config ./config.yaml
```
...
```bash
openclaw protobuf-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw protobuf-go | openclaw task-decomposer
```
