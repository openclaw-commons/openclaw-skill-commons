...
```bash
npx clawhub@latest install wiremock-rust
```
...
```bash
# 使用默认配置
openclaw wiremock-rust

# 查看帮助
openclaw wiremock-rust --help

# 指定配置
openclaw wiremock-rust --config ./config.yaml
```
...
```bash
openclaw wiremock-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw wiremock-rust | openclaw task-decomposer
```
