...
```bash
npx clawhub@latest install kafka-go
```
...
# 查看帮助
openclaw kafka-go --help

# 指定配置
openclaw kafka-go --config ./config.yaml
```
...
```bash
openclaw kafka-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw kafka-go | openclaw task-decomposer
```
