...
```bash
npx clawhub@latest install pnpm-mono
```
...
# 查看帮助
openclaw pnpm-mono --help

# 指定配置
openclaw pnpm-mono --config ./config.yaml
```
...
```bash
openclaw pnpm-mono --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pnpm-mono | openclaw task-decomposer
```
