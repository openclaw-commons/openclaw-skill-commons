...
```bash
npx clawhub@latest install graphql-node
```
...
```bash
# 使用默认配置
openclaw graphql-node

# 查看帮助
openclaw graphql-node --help

# 指定配置
openclaw graphql-node --config ./config.yaml
```
...
```bash
openclaw graphql-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw graphql-node | openclaw task-decomposer
```
