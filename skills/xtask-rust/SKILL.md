...
```bash
npx clawhub@latest install xtask-rust
```
...
# 查看帮助
openclaw xtask-rust --help

# 指定配置
openclaw xtask-rust --config ./config.yaml
```
...
```bash
openclaw xtask-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw xtask-rust | openclaw task-decomposer
```
