...
```bash
npx clawhub@latest install cors-node
```
...
# 查看帮助
openclaw cors-node --help

# 指定配置
openclaw cors-node --config ./config.yaml
```
...
```bash
openclaw cors-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cors-node | openclaw task-decomposer
```
