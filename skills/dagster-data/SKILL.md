...
```bash
npx clawhub@latest install dagster-data
```
...
```bash
# 使用默认配置
openclaw dagster-data

# 查看帮助
openclaw dagster-data --help

# 指定配置
openclaw dagster-data --config ./config.yaml
```
...
```bash
openclaw dagster-data --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw dagster-data | openclaw task-decomposer
```
