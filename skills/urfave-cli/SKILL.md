...
```bash
npx clawhub@latest install urfave-cli
```
...
# 查看帮助
openclaw urfave-cli --help

# 指定配置
openclaw urfave-cli --config ./config.yaml
```
...
```bash
openclaw urfave-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw urfave-cli | openclaw task-decomposer
```
