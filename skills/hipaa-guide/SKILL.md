...
```bash
npx clawhub@latest install hipaa-guide
```
...
```bash
# 使用默认配置
openclaw hipaa-guide

# 查看帮助
openclaw hipaa-guide --help

# 指定配置
openclaw hipaa-guide --config ./config.yaml
```
...
```bash
openclaw hipaa-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw hipaa-guide | openclaw task-decomposer
```
