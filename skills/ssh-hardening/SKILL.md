...
```bash
npx clawhub@latest install ssh-hardening
```
...
# 查看帮助
openclaw ssh-hardening --help

# 指定配置
openclaw ssh-hardening --config ./config.yaml
```
...
```bash
openclaw ssh-hardening --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ssh-hardening | openclaw task-decomposer
```
