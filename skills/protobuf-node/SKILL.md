...
```bash
npx clawhub@latest install protobuf-node
```
...
```bash
# 使用默认配置
openclaw protobuf-node

# 查看帮助
openclaw protobuf-node --help

# 指定配置
openclaw protobuf-node --config ./config.yaml
```
...
```bash
openclaw protobuf-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw protobuf-node | openclaw task-decomposer
```
