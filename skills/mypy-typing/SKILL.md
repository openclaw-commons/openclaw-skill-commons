...
```bash
npx clawhub@latest install mypy-typing
```
...
```bash
# 使用默认配置
openclaw mypy-typing

# 查看帮助
openclaw mypy-typing --help

# 指定配置
openclaw mypy-typing --config ./config.yaml
```
...
```bash
openclaw mypy-typing --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mypy-typing | openclaw task-decomposer
```
