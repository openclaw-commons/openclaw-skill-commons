...
```bash
npx clawhub@latest install unleash-guide
```
...
```bash
# 使用默认配置
openclaw unleash-guide

# 查看帮助
openclaw unleash-guide --help

# 指定配置
openclaw unleash-guide --config ./config.yaml
```
...
```bash
openclaw unleash-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw unleash-guide | openclaw task-decomposer
```
