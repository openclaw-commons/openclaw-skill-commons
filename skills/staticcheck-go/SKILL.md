...
```bash
npx clawhub@latest install staticcheck-go
```
...
```bash
# 使用默认配置
openclaw staticcheck-go

# 查看帮助
openclaw staticcheck-go --help

# 指定配置
openclaw staticcheck-go --config ./config.yaml
```
...
```bash
openclaw staticcheck-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw staticcheck-go | openclaw task-decomposer
```
