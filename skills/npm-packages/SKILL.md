...
```bash
npx clawhub@latest install npm-packages
```
...
# 查看帮助
openclaw npm-packages --help

# 指定配置
openclaw npm-packages --config ./config.yaml
```
...
```bash
openclaw npm-packages --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw npm-packages | openclaw task-decomposer
```
