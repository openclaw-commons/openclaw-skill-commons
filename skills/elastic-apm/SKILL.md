...
```bash
npx clawhub@latest install elastic-apm
```
...
```bash
# 使用默认配置
openclaw elastic-apm

# 查看帮助
openclaw elastic-apm --help

# 指定配置
openclaw elastic-apm --config ./config.yaml
```
...
```bash
openclaw elastic-apm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw elastic-apm | openclaw task-decomposer
```
