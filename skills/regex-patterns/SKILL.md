...
```bash
# 使用默认配置
openclaw regex-patterns

# 查看帮助
openclaw regex-patterns --help

# 指定配置
openclaw regex-patterns --config ./config.yaml
```
...
```bash
openclaw regex-patterns --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw regex-patterns | openclaw task-decomposer
```
