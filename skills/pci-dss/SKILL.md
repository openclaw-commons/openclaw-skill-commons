...
```bash
npx clawhub@latest install pci-dss
```
...
# 查看帮助
openclaw pci-dss --help

# 指定配置
openclaw pci-dss --config ./config.yaml
```
...
```bash
openclaw pci-dss --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pci-dss | openclaw task-decomposer
```
