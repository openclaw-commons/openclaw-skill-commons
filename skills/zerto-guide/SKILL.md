...
```bash
npx clawhub@latest install zerto-guide
```
...
```bash
# 使用默认配置
openclaw zerto-guide

# 查看帮助
openclaw zerto-guide --help

# 指定配置
openclaw zerto-guide --config ./config.yaml
```
...
```bash
openclaw zerto-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw zerto-guide | openclaw task-decomposer
```
