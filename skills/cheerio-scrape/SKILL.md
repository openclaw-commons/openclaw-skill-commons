...
```bash
npx clawhub@latest install cheerio-scrape
```
...
```bash
# 使用默认配置
openclaw cheerio-scrape

# 查看帮助
openclaw cheerio-scrape --help

# 指定配置
openclaw cheerio-scrape --config ./config.yaml
```
...
```bash
openclaw cheerio-scrape --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cheerio-scrape | openclaw task-decomposer
```
