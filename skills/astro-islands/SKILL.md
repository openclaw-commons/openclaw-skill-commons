...
```bash
npx clawhub@latest install astro-islands
```
...
```bash
# 使用默认配置
openclaw astro-islands

# 查看帮助
openclaw astro-islands --help

# 指定配置
openclaw astro-islands --config ./config.yaml
```
...
```bash
openclaw astro-islands --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw astro-islands | openclaw task-decomposer
```
