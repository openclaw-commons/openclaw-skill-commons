...
```bash
npx clawhub@latest install sklearn-ml
```
...
# 查看帮助
openclaw sklearn-ml --help

# 指定配置
openclaw sklearn-ml --config ./config.yaml
```
...
```bash
openclaw sklearn-ml --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sklearn-ml | openclaw task-decomposer
```
