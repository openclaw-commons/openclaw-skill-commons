...
```bash
npx clawhub@latest install istio-advanced
```
...
```bash
# 使用默认配置
openclaw istio-advanced

# 查看帮助
openclaw istio-advanced --help

# 指定配置
openclaw istio-advanced --config ./config.yaml
```
...
```bash
openclaw istio-advanced --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw istio-advanced | openclaw task-decomposer
```
