...
```bash
npx clawhub@latest install upper-db
```
...
# 查看帮助
openclaw upper-db --help

# 指定配置
openclaw upper-db --config ./config.yaml
```
...
```bash
openclaw upper-db --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw upper-db | openclaw task-decomposer
```
