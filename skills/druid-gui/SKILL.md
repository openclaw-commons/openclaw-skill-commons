...
```bash
npx clawhub@latest install druid-gui
```
...
# 查看帮助
openclaw druid-gui --help

# 指定配置
openclaw druid-gui --config ./config.yaml
```
...
```bash
openclaw druid-gui --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw druid-gui | openclaw task-decomposer
```
