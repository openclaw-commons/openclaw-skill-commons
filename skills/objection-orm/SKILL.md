...
```bash
npx clawhub@latest install objection-orm
```
...
```bash
# 使用默认配置
openclaw objection-orm

# 查看帮助
openclaw objection-orm --help

# 指定配置
openclaw objection-orm --config ./config.yaml
```
...
```bash
openclaw objection-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw objection-orm | openclaw task-decomposer
```
