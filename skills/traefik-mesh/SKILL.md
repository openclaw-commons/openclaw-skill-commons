...
```bash
npx clawhub@latest install traefik-mesh
```
...
```bash
# 使用默认配置
openclaw traefik-mesh

# 查看帮助
openclaw traefik-mesh --help

# 指定配置
openclaw traefik-mesh --config ./config.yaml
```
...
```bash
openclaw traefik-mesh --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw traefik-mesh | openclaw task-decomposer
```
