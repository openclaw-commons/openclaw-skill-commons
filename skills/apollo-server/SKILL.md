...
```bash
npx clawhub@latest install apollo-server
```
...
```bash
# 使用默认配置
openclaw apollo-server

# 查看帮助
openclaw apollo-server --help

# 指定配置
openclaw apollo-server --config ./config.yaml
```
...
```bash
openclaw apollo-server --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw apollo-server | openclaw task-decomposer
```
