...
```bash
npx clawhub@latest install rustorm-go
```
...
# 查看帮助
openclaw rustorm-go --help

# 指定配置
openclaw rustorm-go --config ./config.yaml
```
...
```bash
openclaw rustorm-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rustorm-go | openclaw task-decomposer
```
