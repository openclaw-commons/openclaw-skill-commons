...
```bash
npx clawhub@latest install apigee-guide
```
...
```bash
# 使用默认配置
openclaw apigee-guide

# 查看帮助
openclaw apigee-guide --help

# 指定配置
openclaw apigee-guide --config ./config.yaml
```
...
```bash
openclaw apigee-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw apigee-guide | openclaw task-decomposer
```
