...
```bash
npx clawhub@latest install minimax-pdf
```
...
```bash
# 使用默认配置
openclaw minimax-pdf

# 查看帮助
openclaw minimax-pdf --help

# 指定配置
openclaw minimax-pdf --config ./config.yaml
```
...
```bash
openclaw minimax-pdf --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw minimax-pdf | openclaw task-decomposer
```
