...
```bash
npx clawhub@latest install mitchellh-cli
```
...
```bash
# 使用默认配置
openclaw mitchellh-cli

# 查看帮助
openclaw mitchellh-cli --help

# 指定配置
openclaw mitchellh-cli --config ./config.yaml
```
...
```bash
openclaw mitchellh-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mitchellh-cli | openclaw task-decomposer
```
