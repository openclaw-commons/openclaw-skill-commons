...
```bash
npx clawhub@latest install cdn-setup
```
...
# 查看帮助
openclaw cdn-setup --help

# 指定配置
openclaw cdn-setup --config ./config.yaml
```
...
```bash
openclaw cdn-setup --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cdn-setup | openclaw task-decomposer
```
