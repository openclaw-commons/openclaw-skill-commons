...
```bash
npx clawhub@latest install cdn77-guide
```
...
```bash
# 使用默认配置
openclaw cdn77-guide

# 查看帮助
openclaw cdn77-guide --help

# 指定配置
openclaw cdn77-guide --config ./config.yaml
```
...
```bash
openclaw cdn77-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cdn77-guide | openclaw task-decomposer
```
