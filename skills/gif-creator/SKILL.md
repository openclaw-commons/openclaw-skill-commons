...
```bash
npx clawhub@latest install gif-creator
```
...
```bash
# 使用默认配置
openclaw gif-creator

# 查看帮助
openclaw gif-creator --help

# 指定配置
openclaw gif-creator --config ./config.yaml
```
...
```bash
openclaw gif-creator --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gif-creator | openclaw task-decomposer
```
