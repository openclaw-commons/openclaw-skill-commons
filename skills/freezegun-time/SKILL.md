...
```bash
npx clawhub@latest install freezegun-time
```
...
```bash
# 使用默认配置
openclaw freezegun-time

# 查看帮助
openclaw freezegun-time --help

# 指定配置
openclaw freezegun-time --config ./config.yaml
```
...
```bash
openclaw freezegun-time --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw freezegun-time | openclaw task-decomposer
```
