...
```bash
npx clawhub@latest install pytest-mock
```
...
```bash
# 使用默认配置
openclaw pytest-mock

# 查看帮助
openclaw pytest-mock --help

# 指定配置
openclaw pytest-mock --config ./config.yaml
```
...
```bash
openclaw pytest-mock --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pytest-mock | openclaw task-decomposer
```
