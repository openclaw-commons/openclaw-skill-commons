...
```bash
npx clawhub@latest install rate-limit
```
...
# 查看帮助
openclaw rate-limit --help

# 指定配置
openclaw rate-limit --config ./config.yaml
```
...
```bash
openclaw rate-limit --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rate-limit | openclaw task-decomposer
```
