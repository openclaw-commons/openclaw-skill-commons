...
```bash
npx clawhub@latest install color_quant
```
...
```bash
# 使用默认配置
openclaw color_quant

# 查看帮助
openclaw color_quant --help

# 指定配置
openclaw color_quant --config ./config.yaml
```
...
```bash
openclaw color_quant --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw color_quant | openclaw task-decomposer
```
