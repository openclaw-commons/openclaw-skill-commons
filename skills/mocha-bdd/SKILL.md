...
```bash
npx clawhub@latest install mocha-bdd
```
...
# 查看帮助
openclaw mocha-bdd --help

# 指定配置
openclaw mocha-bdd --config ./config.yaml
```
...
```bash
openclaw mocha-bdd --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mocha-bdd | openclaw task-decomposer
```
