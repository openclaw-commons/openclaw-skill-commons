...
```bash
npx clawhub@latest install code-collab
```
...
```bash
# 使用默认配置
openclaw code-collab

# 查看帮助
openclaw code-collab --help

# 指定配置
openclaw code-collab --config ./config.yaml
```
...
```bash
openclaw code-collab --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw code-collab | openclaw task-decomposer
```
