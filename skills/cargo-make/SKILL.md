...
```bash
npx clawhub@latest install cargo-make
```
...
# 查看帮助
openclaw cargo-make --help

# 指定配置
openclaw cargo-make --config ./config.yaml
```
...
```bash
openclaw cargo-make --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cargo-make | openclaw task-decomposer
```
