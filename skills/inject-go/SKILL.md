...
```bash
npx clawhub@latest install inject-go
```
...
# 查看帮助
openclaw inject-go --help

# 指定配置
openclaw inject-go --config ./config.yaml
```
...
```bash
openclaw inject-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw inject-go | openclaw task-decomposer
```
