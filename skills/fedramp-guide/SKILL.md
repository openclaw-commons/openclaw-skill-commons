...
```bash
npx clawhub@latest install fedramp-guide
```
...
```bash
# 使用默认配置
openclaw fedramp-guide

# 查看帮助
openclaw fedramp-guide --help

# 指定配置
openclaw fedramp-guide --config ./config.yaml
```
...
```bash
openclaw fedramp-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw fedramp-guide | openclaw task-decomposer
```
