...
```bash
npx clawhub@latest install scrapy-crawler
```
...
```bash
# 使用默认配置
openclaw scrapy-crawler

# 查看帮助
openclaw scrapy-crawler --help

# 指定配置
openclaw scrapy-crawler --config ./config.yaml
```
...
```bash
openclaw scrapy-crawler --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw scrapy-crawler | openclaw task-decomposer
```
