...
```bash
npx clawhub@latest install emqx-guide
```
...
# 查看帮助
openclaw emqx-guide --help

# 指定配置
openclaw emqx-guide --config ./config.yaml
```
...
```bash
openclaw emqx-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw emqx-guide | openclaw task-decomposer
```
