...
```bash
npx clawhub@latest install rustls-tls
```
...
# 查看帮助
openclaw rustls-tls --help

# 指定配置
openclaw rustls-tls --config ./config.yaml
```
...
```bash
openclaw rustls-tls --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rustls-tls | openclaw task-decomposer
```
