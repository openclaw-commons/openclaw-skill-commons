...
```bash
# 使用默认配置
openclaw git-workflows

# 查看帮助
openclaw git-workflows --help

# 指定配置
openclaw git-workflows --config ./config.yaml
```
...
```bash
openclaw git-workflows --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw git-workflows | openclaw task-decomposer
```
