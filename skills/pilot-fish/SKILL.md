...
```bash
npx clawhub@latest install pilot-fish
```
...
# 查看帮助
openclaw pilot-fish --help

# 指定配置
openclaw pilot-fish --config ./config.yaml
```
...
```bash
openclaw pilot-fish --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pilot-fish | openclaw task-decomposer
```
