...
```bash
npx clawhub@latest install veracode-guide
```
...
```bash
# 使用默认配置
openclaw veracode-guide

# 查看帮助
openclaw veracode-guide --help

# 指定配置
openclaw veracode-guide --config ./config.yaml
```
...
```bash
openclaw veracode-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw veracode-guide | openclaw task-decomposer
```
