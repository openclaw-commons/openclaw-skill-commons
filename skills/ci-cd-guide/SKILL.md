...
```bash
npx clawhub@latest install ci-cd-guide
```
...
```bash
# 使用默认配置
openclaw ci-cd-guide

# 查看帮助
openclaw ci-cd-guide --help

# 指定配置
openclaw ci-cd-guide --config ./config.yaml
```
...
```bash
openclaw ci-cd-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ci-cd-guide | openclaw task-decomposer
```
