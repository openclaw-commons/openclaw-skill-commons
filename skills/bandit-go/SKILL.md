...
```bash
npx clawhub@latest install bandit-go
```
...
# 查看帮助
openclaw bandit-go --help

# 指定配置
openclaw bandit-go --config ./config.yaml
```
...
```bash
openclaw bandit-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bandit-go | openclaw task-decomposer
```
