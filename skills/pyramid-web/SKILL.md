...
```bash
npx clawhub@latest install pyramid-web
```
...
```bash
# 使用默认配置
openclaw pyramid-web

# 查看帮助
openclaw pyramid-web --help

# 指定配置
openclaw pyramid-web --config ./config.yaml
```
...
```bash
openclaw pyramid-web --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pyramid-web | openclaw task-decomposer
```
