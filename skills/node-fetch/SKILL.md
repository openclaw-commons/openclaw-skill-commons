...
```bash
npx clawhub@latest install node-fetch
```
...
# 查看帮助
openclaw node-fetch --help

# 指定配置
openclaw node-fetch --config ./config.yaml
```
...
```bash
openclaw node-fetch --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw node-fetch | openclaw task-decomposer
```
