...
```bash
npx clawhub@latest install listr-tasks
```
...
```bash
# 使用默认配置
openclaw listr-tasks

# 查看帮助
openclaw listr-tasks --help

# 指定配置
openclaw listr-tasks --config ./config.yaml
```
...
```bash
openclaw listr-tasks --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw listr-tasks | openclaw task-decomposer
```
