...
```bash
npx clawhub@latest install dotenv-config
```
...
```bash
# 使用默认配置
openclaw dotenv-config

# 查看帮助
openclaw dotenv-config --help

# 指定配置
openclaw dotenv-config --config ./config.yaml
```
...
```bash
openclaw dotenv-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw dotenv-config | openclaw task-decomposer
```
