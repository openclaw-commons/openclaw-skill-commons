...
```bash
npx clawhub@latest install serde-rust
```
...
# 查看帮助
openclaw serde-rust --help

# 指定配置
openclaw serde-rust --config ./config.yaml
```
...
```bash
openclaw serde-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw serde-rust | openclaw task-decomposer
```
