...
```bash
npx clawhub@latest install cdk-go
```
...
# 查看帮助
openclaw cdk-go --help

# 指定配置
openclaw cdk-go --config ./config.yaml
```
...
```bash
openclaw cdk-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cdk-go | openclaw task-decomposer
```
