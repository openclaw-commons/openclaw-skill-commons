...
```bash
npx clawhub@latest install structopt-cli
```
...
```bash
# 使用默认配置
openclaw structopt-cli

# 查看帮助
openclaw structopt-cli --help

# 指定配置
openclaw structopt-cli --config ./config.yaml
```
...
```bash
openclaw structopt-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw structopt-cli | openclaw task-decomposer
```
