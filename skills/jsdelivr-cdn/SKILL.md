...
```bash
npx clawhub@latest install jsdelivr-cdn
```
...
```bash
# 使用默认配置
openclaw jsdelivr-cdn

# 查看帮助
openclaw jsdelivr-cdn --help

# 指定配置
openclaw jsdelivr-cdn --config ./config.yaml
```
...
```bash
openclaw jsdelivr-cdn --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw jsdelivr-cdn | openclaw task-decomposer
```
