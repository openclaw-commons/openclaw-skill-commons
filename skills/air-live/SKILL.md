...
```bash
npx clawhub@latest install air-live
```
...
# 查看帮助
openclaw air-live --help

# 指定配置
openclaw air-live --config ./config.yaml
```
...
```bash
openclaw air-live --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw air-live | openclaw task-decomposer
```
