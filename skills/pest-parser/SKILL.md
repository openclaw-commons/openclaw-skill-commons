...
```bash
npx clawhub@latest install pest-parser
```
...
```bash
# 使用默认配置
openclaw pest-parser

# 查看帮助
openclaw pest-parser --help

# 指定配置
openclaw pest-parser --config ./config.yaml
```
...
```bash
openclaw pest-parser --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pest-parser | openclaw task-decomposer
```
