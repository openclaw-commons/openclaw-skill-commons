...
```bash
npx clawhub@latest install snyk-advanced
```
...
```bash
# 使用默认配置
openclaw snyk-advanced

# 查看帮助
openclaw snyk-advanced --help

# 指定配置
openclaw snyk-advanced --config ./config.yaml
```
...
```bash
openclaw snyk-advanced --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw snyk-advanced | openclaw task-decomposer
```
