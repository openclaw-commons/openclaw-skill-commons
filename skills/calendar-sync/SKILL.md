...
# 查看帮助
openclaw calendar-sync --help

# 指定配置
openclaw calendar-sync --config ./config.yaml
```
...
```bash
openclaw calendar-sync --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw calendar-sync | openclaw task-decomposer
```
