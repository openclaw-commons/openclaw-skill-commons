...
```bash
npx clawhub@latest install httpmock-rust
```
...
```bash
# 使用默认配置
openclaw httpmock-rust

# 查看帮助
openclaw httpmock-rust --help

# 指定配置
openclaw httpmock-rust --config ./config.yaml
```
...
```bash
openclaw httpmock-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw httpmock-rust | openclaw task-decomposer
```
