...
```bash
npx clawhub@latest install aws-cloudfront
```
...
```bash
# 使用默认配置
openclaw aws-cloudfront

# 查看帮助
openclaw aws-cloudfront --help

# 指定配置
openclaw aws-cloudfront --config ./config.yaml
```
...
```bash
openclaw aws-cloudfront --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw aws-cloudfront | openclaw task-decomposer
```
