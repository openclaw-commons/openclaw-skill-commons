...
```bash
npx clawhub@latest install gcp-python
```
...
# 查看帮助
openclaw gcp-python --help

# 指定配置
openclaw gcp-python --config ./config.yaml
```
...
```bash
openclaw gcp-python --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gcp-python | openclaw task-decomposer
```
