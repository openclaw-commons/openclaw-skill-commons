...
```bash
npx clawhub@latest install cordova-guide
```
...
```bash
# 使用默认配置
openclaw cordova-guide

# 查看帮助
openclaw cordova-guide --help

# 指定配置
openclaw cordova-guide --config ./config.yaml
```
...
```bash
openclaw cordova-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cordova-guide | openclaw task-decomposer
```
