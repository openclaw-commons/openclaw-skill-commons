...
# 查看帮助
openclaw agent-memory --help

# 指定配置
openclaw agent-memory --config ./config.yaml
```
...
```bash
openclaw agent-memory --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw agent-memory | openclaw task-decomposer
```
