...
```bash
npx clawhub@latest install grpc-go
```
...
# 查看帮助
openclaw grpc-go --help

# 指定配置
openclaw grpc-go --config ./config.yaml
```
...
```bash
openclaw grpc-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw grpc-go | openclaw task-decomposer
```
