...
```bash
npx clawhub@latest install image-rust
```
...
# 查看帮助
openclaw image-rust --help

# 指定配置
openclaw image-rust --config ./config.yaml
```
...
```bash
openclaw image-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw image-rust | openclaw task-decomposer
```
