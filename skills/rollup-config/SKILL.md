...
```bash
npx clawhub@latest install rollup-config
```
...
```bash
# 使用默认配置
openclaw rollup-config

# 查看帮助
openclaw rollup-config --help

# 指定配置
openclaw rollup-config --config ./config.yaml
```
...
```bash
openclaw rollup-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rollup-config | openclaw task-decomposer
```
