...
```bash
npx clawhub@latest install redis-advanced
```
...
```bash
# 使用默认配置
openclaw redis-advanced

# 查看帮助
openclaw redis-advanced --help

# 指定配置
openclaw redis-advanced --config ./config.yaml
```
...
```bash
openclaw redis-advanced --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw redis-advanced | openclaw task-decomposer
```
