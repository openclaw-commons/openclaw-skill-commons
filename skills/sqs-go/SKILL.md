...
```bash
npx clawhub@latest install sqs-go
```
...
# 查看帮助
openclaw sqs-go --help

# 指定配置
openclaw sqs-go --config ./config.yaml
```
...
```bash
openclaw sqs-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sqs-go | openclaw task-decomposer
```
