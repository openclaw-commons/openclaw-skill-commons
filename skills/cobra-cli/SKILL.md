...
```bash
npx clawhub@latest install cobra-cli
```
...
# 查看帮助
openclaw cobra-cli --help

# 指定配置
openclaw cobra-cli --config ./config.yaml
```
...
```bash
openclaw cobra-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cobra-cli | openclaw task-decomposer
```
