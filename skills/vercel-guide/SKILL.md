...
```bash
npx clawhub@latest install vercel-guide
```
...
```bash
# 使用默认配置
openclaw vercel-guide

# 查看帮助
openclaw vercel-guide --help

# 指定配置
openclaw vercel-guide --config ./config.yaml
```
...
```bash
openclaw vercel-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw vercel-guide | openclaw task-decomposer
```
