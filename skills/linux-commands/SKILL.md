...
```bash
npx clawhub@latest install linux-commands
```
...
```bash
# 使用默认配置
openclaw linux-commands

# 查看帮助
openclaw linux-commands --help

# 指定配置
openclaw linux-commands --config ./config.yaml
```
...
```bash
openclaw linux-commands --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw linux-commands | openclaw task-decomposer
```
