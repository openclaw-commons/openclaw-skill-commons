...
```bash
npx clawhub@latest install vault-go
```
...
# 查看帮助
openclaw vault-go --help

# 指定配置
openclaw vault-go --config ./config.yaml
```
...
```bash
openclaw vault-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw vault-go | openclaw task-decomposer
```
