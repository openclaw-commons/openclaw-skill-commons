...
```bash
npx clawhub@latest install code-mentor
```
...
```bash
# 使用默认配置
openclaw code-mentor

# 查看帮助
openclaw code-mentor --help

# 指定配置
openclaw code-mentor --config ./config.yaml
```
...
```bash
openclaw code-mentor --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw code-mentor | openclaw task-decomposer
```
