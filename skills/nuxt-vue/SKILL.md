...
```bash
npx clawhub@latest install nuxt-vue
```
...
# 查看帮助
openclaw nuxt-vue --help

# 指定配置
openclaw nuxt-vue --config ./config.yaml
```
...
```bash
openclaw nuxt-vue --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nuxt-vue | openclaw task-decomposer
```
