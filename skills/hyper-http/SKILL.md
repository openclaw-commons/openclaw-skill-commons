...
```bash
npx clawhub@latest install hyper-http
```
...
# 查看帮助
openclaw hyper-http --help

# 指定配置
openclaw hyper-http --config ./config.yaml
```
...
```bash
openclaw hyper-http --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw hyper-http | openclaw task-decomposer
```
