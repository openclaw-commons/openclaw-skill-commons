...
```bash
npx clawhub@latest install goal-setting
```
...
```bash
# 使用默认配置
openclaw goal-setting

# 查看帮助
openclaw goal-setting --help

# 指定配置
openclaw goal-setting --config ./config.yaml
```
...
```bash
openclaw goal-setting --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw goal-setting | openclaw task-decomposer
```
