...
```bash
npx clawhub@latest install docker-python
```
...
```bash
# 使用默认配置
openclaw docker-python

# 查看帮助
openclaw docker-python --help

# 指定配置
openclaw docker-python --config ./config.yaml
```
...
```bash
openclaw docker-python --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw docker-python | openclaw task-decomposer
```
