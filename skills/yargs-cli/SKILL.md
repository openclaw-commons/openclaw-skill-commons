...
```bash
npx clawhub@latest install yargs-cli
```
...
# 查看帮助
openclaw yargs-cli --help

# 指定配置
openclaw yargs-cli --config ./config.yaml
```
...
```bash
openclaw yargs-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw yargs-cli | openclaw task-decomposer
```
