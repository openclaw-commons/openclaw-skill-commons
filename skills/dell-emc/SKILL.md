...
```bash
npx clawhub@latest install dell-emc
```
...
# 查看帮助
openclaw dell-emc --help

# 指定配置
openclaw dell-emc --config ./config.yaml
```
...
```bash
openclaw dell-emc --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw dell-emc | openclaw task-decomposer
```
