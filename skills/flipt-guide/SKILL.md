...
```bash
npx clawhub@latest install flipt-guide
```
...
```bash
# 使用默认配置
openclaw flipt-guide

# 查看帮助
openclaw flipt-guide --help

# 指定配置
openclaw flipt-guide --config ./config.yaml
```
...
```bash
openclaw flipt-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw flipt-guide | openclaw task-decomposer
```
