...
```bash
npx clawhub@latest install jimp-image
```
...
# 查看帮助
openclaw jimp-image --help

# 指定配置
openclaw jimp-image --config ./config.yaml
```
...
```bash
openclaw jimp-image --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw jimp-image | openclaw task-decomposer
```
