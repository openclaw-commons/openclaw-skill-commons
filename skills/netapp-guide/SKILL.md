...
```bash
npx clawhub@latest install netapp-guide
```
...
```bash
# 使用默认配置
openclaw netapp-guide

# 查看帮助
openclaw netapp-guide --help

# 指定配置
openclaw netapp-guide --config ./config.yaml
```
...
```bash
openclaw netapp-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw netapp-guide | openclaw task-decomposer
```
