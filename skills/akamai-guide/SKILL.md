...
```bash
npx clawhub@latest install akamai-guide
```
...
```bash
# 使用默认配置
openclaw akamai-guide

# 查看帮助
openclaw akamai-guide --help

# 指定配置
openclaw akamai-guide --config ./config.yaml
```
...
```bash
openclaw akamai-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw akamai-guide | openclaw task-decomposer
```
