...
```bash
# 使用默认配置
openclaw minimax-docx

# 查看帮助
openclaw minimax-docx --help

# 指定配置
openclaw minimax-docx --config ./config.yaml
```
...
```bash
openclaw minimax-docx --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw minimax-docx | openclaw task-decomposer
```
