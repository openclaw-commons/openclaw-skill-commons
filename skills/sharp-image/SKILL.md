...
```bash
npx clawhub@latest install sharp-image
```
...
```bash
# 使用默认配置
openclaw sharp-image

# 查看帮助
openclaw sharp-image --help

# 指定配置
openclaw sharp-image --config ./config.yaml
```
...
```bash
openclaw sharp-image --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sharp-image | openclaw task-decomposer
```
