...
```bash
npx clawhub@latest install neutralinojs
```
...
```bash
# 使用默认配置
openclaw neutralinojs

# 查看帮助
openclaw neutralinojs --help

# 指定配置
openclaw neutralinojs --config ./config.yaml
```
...
```bash
openclaw neutralinojs --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw neutralinojs | openclaw task-decomposer
```
