...
```bash
npx clawhub@latest install fortify-guide
```
...
```bash
# 使用默认配置
openclaw fortify-guide

# 查看帮助
openclaw fortify-guide --help

# 指定配置
openclaw fortify-guide --config ./config.yaml
```
...
```bash
openclaw fortify-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw fortify-guide | openclaw task-decomposer
```
