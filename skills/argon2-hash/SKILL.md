...
```bash
npx clawhub@latest install argon2-hash
```
...
```bash
# 使用默认配置
openclaw argon2-hash

# 查看帮助
openclaw argon2-hash --help

# 指定配置
openclaw argon2-hash --config ./config.yaml
```
...
```bash
openclaw argon2-hash --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw argon2-hash | openclaw task-decomposer
```
