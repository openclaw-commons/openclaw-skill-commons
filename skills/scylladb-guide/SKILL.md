...
```bash
npx clawhub@latest install scylladb-guide
```
...
```bash
# 使用默认配置
openclaw scylladb-guide

# 查看帮助
openclaw scylladb-guide --help

# 指定配置
openclaw scylladb-guide --config ./config.yaml
```
...
```bash
openclaw scylladb-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw scylladb-guide | openclaw task-decomposer
```
