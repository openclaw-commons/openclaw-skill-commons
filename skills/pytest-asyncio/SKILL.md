...
```bash
npx clawhub@latest install pytest-asyncio
```
...
```bash
# 使用默认配置
openclaw pytest-asyncio

# 查看帮助
openclaw pytest-asyncio --help

# 指定配置
openclaw pytest-asyncio --config ./config.yaml
```
...
```bash
openclaw pytest-asyncio --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pytest-asyncio | openclaw task-decomposer
```
