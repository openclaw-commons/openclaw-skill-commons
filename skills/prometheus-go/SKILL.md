...
```bash
npx clawhub@latest install prometheus-go
```
...
```bash
# 使用默认配置
openclaw prometheus-go

# 查看帮助
openclaw prometheus-go --help

# 指定配置
openclaw prometheus-go --config ./config.yaml
```
...
```bash
openclaw prometheus-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw prometheus-go | openclaw task-decomposer
```
