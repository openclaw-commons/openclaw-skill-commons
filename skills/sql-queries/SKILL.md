...
```bash
npx clawhub@latest install sql-queries
```
...
```bash
# 使用默认配置
openclaw sql-queries

# 查看帮助
openclaw sql-queries --help

# 指定配置
openclaw sql-queries --config ./config.yaml
```
...
```bash
openclaw sql-queries --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sql-queries | openclaw task-decomposer
```
