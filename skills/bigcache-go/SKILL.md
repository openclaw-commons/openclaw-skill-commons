...
```bash
npx clawhub@latest install bigcache-go
```
...
```bash
# 使用默认配置
openclaw bigcache-go

# 查看帮助
openclaw bigcache-go --help

# 指定配置
openclaw bigcache-go --config ./config.yaml
```
...
```bash
openclaw bigcache-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bigcache-go | openclaw task-decomposer
```
