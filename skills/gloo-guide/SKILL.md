...
```bash
npx clawhub@latest install gloo-guide
```
...
# 查看帮助
openclaw gloo-guide --help

# 指定配置
openclaw gloo-guide --config ./config.yaml
```
...
```bash
openclaw gloo-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gloo-guide | openclaw task-decomposer
```
