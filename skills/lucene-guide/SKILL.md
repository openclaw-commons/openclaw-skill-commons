...
```bash
npx clawhub@latest install lucene-guide
```
...
```bash
# 使用默认配置
openclaw lucene-guide

# 查看帮助
openclaw lucene-guide --help

# 指定配置
openclaw lucene-guide --config ./config.yaml
```
...
```bash
openclaw lucene-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw lucene-guide | openclaw task-decomposer
```
