...
```bash
npx clawhub@latest install aks-guide
```
...
# 查看帮助
openclaw aks-guide --help

# 指定配置
openclaw aks-guide --config ./config.yaml
```
...
```bash
openclaw aks-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw aks-guide | openclaw task-decomposer
```
