...
```bash
npx clawhub@latest install cilium-guide
```
...
```bash
# 使用默认配置
openclaw cilium-guide

# 查看帮助
openclaw cilium-guide --help

# 指定配置
openclaw cilium-guide --config ./config.yaml
```
...
```bash
openclaw cilium-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cilium-guide | openclaw task-decomposer
```
