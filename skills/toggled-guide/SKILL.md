...
```bash
npx clawhub@latest install toggled-guide
```
...
```bash
# 使用默认配置
openclaw toggled-guide

# 查看帮助
openclaw toggled-guide --help

# 指定配置
openclaw toggled-guide --config ./config.yaml
```
...
```bash
openclaw toggled-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw toggled-guide | openclaw task-decomposer
```
