...
```bash
npx clawhub@latest install begin-guide
```
...
```bash
# 使用默认配置
openclaw begin-guide

# 查看帮助
openclaw begin-guide --help

# 指定配置
openclaw begin-guide --config ./config.yaml
```
...
```bash
openclaw begin-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw begin-guide | openclaw task-decomposer
```
