...
```bash
npx clawhub@latest install transcript
```
...
# 查看帮助
openclaw transcript --help

# 指定配置
openclaw transcript --config ./config.yaml
```
...
```bash
openclaw transcript --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw transcript | openclaw task-decomposer
```
