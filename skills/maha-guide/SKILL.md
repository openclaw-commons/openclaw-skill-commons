...
```bash
npx clawhub@latest install maha-guide
```
...
# 查看帮助
openclaw maha-guide --help

# 指定配置
openclaw maha-guide --config ./config.yaml
```
...
```bash
openclaw maha-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw maha-guide | openclaw task-decomposer
```
