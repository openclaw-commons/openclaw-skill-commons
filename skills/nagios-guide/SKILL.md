...
```bash
npx clawhub@latest install nagios-guide
```
...
```bash
# 使用默认配置
openclaw nagios-guide

# 查看帮助
openclaw nagios-guide --help

# 指定配置
openclaw nagios-guide --config ./config.yaml
```
...
```bash
openclaw nagios-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nagios-guide | openclaw task-decomposer
```
