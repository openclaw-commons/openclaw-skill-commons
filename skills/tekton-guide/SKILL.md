...
```bash
npx clawhub@latest install tekton-guide
```
...
```bash
# 使用默认配置
openclaw tekton-guide

# 查看帮助
openclaw tekton-guide --help

# 指定配置
openclaw tekton-guide --config ./config.yaml
```
...
```bash
openclaw tekton-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tekton-guide | openclaw task-decomposer
```
