...
```bash
npx clawhub@latest install krakend-guide
```
...
```bash
# 使用默认配置
openclaw krakend-guide

# 查看帮助
openclaw krakend-guide --help

# 指定配置
openclaw krakend-guide --config ./config.yaml
```
...
```bash
openclaw krakend-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw krakend-guide | openclaw task-decomposer
```
