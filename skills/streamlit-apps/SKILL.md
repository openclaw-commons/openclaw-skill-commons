...
```bash
npx clawhub@latest install streamlit-apps
```
...
```bash
# 使用默认配置
openclaw streamlit-apps

# 查看帮助
openclaw streamlit-apps --help

# 指定配置
openclaw streamlit-apps --config ./config.yaml
```
...
```bash
openclaw streamlit-apps --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw streamlit-apps | openclaw task-decomposer
```
