...
```bash
npx clawhub@latest install eslint-quality
```
...
```bash
# 使用默认配置
openclaw eslint-quality

# 查看帮助
openclaw eslint-quality --help

# 指定配置
openclaw eslint-quality --config ./config.yaml
```
...
```bash
openclaw eslint-quality --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw eslint-quality | openclaw task-decomposer
```
