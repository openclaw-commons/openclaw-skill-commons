...
```bash
npx clawhub@latest install cdnjs-guide
```
...
```bash
# 使用默认配置
openclaw cdnjs-guide

# 查看帮助
openclaw cdnjs-guide --help

# 指定配置
openclaw cdnjs-guide --config ./config.yaml
```
...
```bash
openclaw cdnjs-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cdnjs-guide | openclaw task-decomposer
```
