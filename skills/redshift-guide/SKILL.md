...
```bash
npx clawhub@latest install redshift-guide
```
...
```bash
# 使用默认配置
openclaw redshift-guide

# 查看帮助
openclaw redshift-guide --help

# 指定配置
openclaw redshift-guide --config ./config.yaml
```
...
```bash
openclaw redshift-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw redshift-guide | openclaw task-decomposer
```
