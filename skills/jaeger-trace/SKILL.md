...
```bash
npx clawhub@latest install jaeger-trace
```
...
```bash
# 使用默认配置
openclaw jaeger-trace

# 查看帮助
openclaw jaeger-trace --help

# 指定配置
openclaw jaeger-trace --config ./config.yaml
```
...
```bash
openclaw jaeger-trace --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw jaeger-trace | openclaw task-decomposer
```
