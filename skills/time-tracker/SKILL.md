...
```bash
npx clawhub@latest install time-tracker
```
...
```bash
# 使用默认配置
openclaw time-tracker

# 查看帮助
openclaw time-tracker --help

# 指定配置
openclaw time-tracker --config ./config.yaml
```
...
```bash
openclaw time-tracker --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw time-tracker | openclaw task-decomposer
```
