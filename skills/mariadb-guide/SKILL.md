...
```bash
npx clawhub@latest install mariadb-guide
```
...
```bash
# 使用默认配置
openclaw mariadb-guide

# 查看帮助
openclaw mariadb-guide --help

# 指定配置
openclaw mariadb-guide --config ./config.yaml
```
...
```bash
openclaw mariadb-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mariadb-guide | openclaw task-decomposer
```
