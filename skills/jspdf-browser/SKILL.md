...
```bash
npx clawhub@latest install jspdf-browser
```
...
```bash
# 使用默认配置
openclaw jspdf-browser

# 查看帮助
openclaw jspdf-browser --help

# 指定配置
openclaw jspdf-browser --config ./config.yaml
```
...
```bash
openclaw jspdf-browser --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw jspdf-browser | openclaw task-decomposer
```
