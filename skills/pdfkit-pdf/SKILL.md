...
```bash
npx clawhub@latest install pdfkit-pdf
```
...
# 查看帮助
openclaw pdfkit-pdf --help

# 指定配置
openclaw pdfkit-pdf --config ./config.yaml
```
...
```bash
openclaw pdfkit-pdf --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pdfkit-pdf | openclaw task-decomposer
```
