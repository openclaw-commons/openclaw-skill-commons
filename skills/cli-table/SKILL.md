...
```bash
npx clawhub@latest install cli-table
```
...
# 查看帮助
openclaw cli-table --help

# 指定配置
openclaw cli-table --config ./config.yaml
```
...
```bash
openclaw cli-table --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cli-table | openclaw task-decomposer
```
