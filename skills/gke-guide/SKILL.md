...
```bash
npx clawhub@latest install gke-guide
```
...
# 查看帮助
openclaw gke-guide --help

# 指定配置
openclaw gke-guide --config ./config.yaml
```
...
```bash
openclaw gke-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gke-guide | openclaw task-decomposer
```
