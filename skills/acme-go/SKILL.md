...
```bash
npx clawhub@latest install acme-go
```
...
# 查看帮助
openclaw acme-go --help

# 指定配置
openclaw acme-go --config ./config.yaml
```
...
```bash
openclaw acme-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw acme-go | openclaw task-decomposer
```
