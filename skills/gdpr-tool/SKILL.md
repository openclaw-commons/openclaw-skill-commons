...
```bash
npx clawhub@latest install gdpr-tool
```
...
# 查看帮助
openclaw gdpr-tool --help

# 指定配置
openclaw gdpr-tool --config ./config.yaml
```
...
```bash
openclaw gdpr-tool --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gdpr-tool | openclaw task-decomposer
```
