...
```bash
npx clawhub@latest install indicatif-rust
```
...
```bash
# 使用默认配置
openclaw indicatif-rust

# 查看帮助
openclaw indicatif-rust --help

# 指定配置
openclaw indicatif-rust --config ./config.yaml
```
...
```bash
openclaw indicatif-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw indicatif-rust | openclaw task-decomposer
```
