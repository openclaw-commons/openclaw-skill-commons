...
```bash
npx clawhub@latest install gravitee-guide
```
...
```bash
# 使用默认配置
openclaw gravitee-guide

# 查看帮助
openclaw gravitee-guide --help

# 指定配置
openclaw gravitee-guide --config ./config.yaml
```
...
```bash
openclaw gravitee-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gravitee-guide | openclaw task-decomposer
```
