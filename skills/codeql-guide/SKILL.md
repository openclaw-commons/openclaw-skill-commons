...
```bash
npx clawhub@latest install codeql-guide
```
...
```bash
# 使用默认配置
openclaw codeql-guide

# 查看帮助
openclaw codeql-guide --help

# 指定配置
openclaw codeql-guide --config ./config.yaml
```
...
```bash
openclaw codeql-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw codeql-guide | openclaw task-decomposer
```
