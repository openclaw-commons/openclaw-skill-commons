...
```bash
npx clawhub@latest install keras-neural
```
...
```bash
# 使用默认配置
openclaw keras-neural

# 查看帮助
openclaw keras-neural --help

# 指定配置
openclaw keras-neural --config ./config.yaml
```
...
```bash
openclaw keras-neural --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw keras-neural | openclaw task-decomposer
```
