...
```bash
npx clawhub@latest install flatbuffers
```
...
```bash
# 使用默认配置
openclaw flatbuffers

# 查看帮助
openclaw flatbuffers --help

# 指定配置
openclaw flatbuffers --config ./config.yaml
```
...
```bash
openclaw flatbuffers --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw flatbuffers | openclaw task-decomposer
```
