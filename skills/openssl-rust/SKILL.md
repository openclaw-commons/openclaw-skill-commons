...
```bash
npx clawhub@latest install openssl-rust
```
...
```bash
# 使用默认配置
openclaw openssl-rust

# 查看帮助
openclaw openssl-rust --help

# 指定配置
openclaw openssl-rust --config ./config.yaml
```
...
```bash
openclaw openssl-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw openssl-rust | openclaw task-decomposer
```
