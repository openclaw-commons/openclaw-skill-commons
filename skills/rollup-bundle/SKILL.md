...
```bash
npx clawhub@latest install rollup-bundle
```
...
```bash
# 使用默认配置
openclaw rollup-bundle

# 查看帮助
openclaw rollup-bundle --help

# 指定配置
openclaw rollup-bundle --config ./config.yaml
```
...
```bash
openclaw rollup-bundle --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rollup-bundle | openclaw task-decomposer
```
