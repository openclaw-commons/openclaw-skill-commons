...
```bash
npx clawhub@latest install test-coverage
```
...
# 查看帮助
openclaw test-coverage --help

# 指定配置
openclaw test-coverage --config ./config.yaml
```
...
```bash
openclaw test-coverage --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw test-coverage | openclaw task-decomposer
```
