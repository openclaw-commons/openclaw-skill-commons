...
```bash
npx clawhub@latest install pino-logging
```
...
```bash
# 使用默认配置
openclaw pino-logging

# 查看帮助
openclaw pino-logging --help

# 指定配置
openclaw pino-logging --config ./config.yaml
```
...
```bash
openclaw pino-logging --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pino-logging | openclaw task-decomposer
```
