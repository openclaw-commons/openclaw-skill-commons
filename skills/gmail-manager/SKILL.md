...
```bash
# 使用默认配置
openclaw gmail-manager

# 查看帮助
openclaw gmail-manager --help

# 指定配置
openclaw gmail-manager --config ./config.yaml
```
...
```bash
openclaw gmail-manager --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gmail-manager | openclaw task-decomposer
```
