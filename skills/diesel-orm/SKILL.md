...
```bash
npx clawhub@latest install diesel-orm
```
...
# 查看帮助
openclaw diesel-orm --help

# 指定配置
openclaw diesel-orm --config ./config.yaml
```
...
```bash
openclaw diesel-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw diesel-orm | openclaw task-decomposer
```
