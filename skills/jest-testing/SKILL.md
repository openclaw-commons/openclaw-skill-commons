...
```bash
npx clawhub@latest install jest-testing
```
...
```bash
# 使用默认配置
openclaw jest-testing

# 查看帮助
openclaw jest-testing --help

# 指定配置
openclaw jest-testing --config ./config.yaml
```
...
```bash
openclaw jest-testing --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw jest-testing | openclaw task-decomposer
```
