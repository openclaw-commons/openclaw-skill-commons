...
```bash
npx clawhub@latest install memcached-node
```
...
```bash
# 使用默认配置
openclaw memcached-node

# 查看帮助
openclaw memcached-node --help

# 指定配置
openclaw memcached-node --config ./config.yaml
```
...
```bash
openclaw memcached-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw memcached-node | openclaw task-decomposer
```
