...
```bash
npx clawhub@latest install azure-python
```
...
```bash
# 使用默认配置
openclaw azure-python

# 查看帮助
openclaw azure-python --help

# 指定配置
openclaw azure-python --config ./config.yaml
```
...
```bash
openclaw azure-python --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw azure-python | openclaw task-decomposer
```
