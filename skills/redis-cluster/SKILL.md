...
```bash
npx clawhub@latest install redis-cluster
```
...
```bash
# 使用默认配置
openclaw redis-cluster

# 查看帮助
openclaw redis-cluster --help

# 指定配置
openclaw redis-cluster --config ./config.yaml
```
...
```bash
openclaw redis-cluster --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw redis-cluster | openclaw task-decomposer
```
