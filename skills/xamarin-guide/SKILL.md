...
```bash
npx clawhub@latest install xamarin-guide
```
...
```bash
# 使用默认配置
openclaw xamarin-guide

# 查看帮助
openclaw xamarin-guide --help

# 指定配置
openclaw xamarin-guide --config ./config.yaml
```
...
```bash
openclaw xamarin-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw xamarin-guide | openclaw task-decomposer
```
