...
```bash
npx clawhub@latest install wasm-bindgen
```
...
```bash
# 使用默认配置
openclaw wasm-bindgen

# 查看帮助
openclaw wasm-bindgen --help

# 指定配置
openclaw wasm-bindgen --config ./config.yaml
```
...
```bash
openclaw wasm-bindgen --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw wasm-bindgen | openclaw task-decomposer
```
