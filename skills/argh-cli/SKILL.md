...
```bash
npx clawhub@latest install argh-cli
```
...
# 查看帮助
openclaw argh-cli --help

# 指定配置
openclaw argh-cli --config ./config.yaml
```
...
```bash
openclaw argh-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw argh-cli | openclaw task-decomposer
```
