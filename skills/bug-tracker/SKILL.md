...
```bash
npx clawhub@latest install bug-tracker
```
...
```bash
# 使用默认配置
openclaw bug-tracker

# 查看帮助
openclaw bug-tracker --help

# 指定配置
openclaw bug-tracker --config ./config.yaml
```
...
```bash
openclaw bug-tracker --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bug-tracker | openclaw task-decomposer
```
