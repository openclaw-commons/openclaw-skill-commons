...
```bash
npx clawhub@latest install dynamodb-guide
```
...
```bash
# 使用默认配置
openclaw dynamodb-guide

# 查看帮助
openclaw dynamodb-guide --help

# 指定配置
openclaw dynamodb-guide --config ./config.yaml
```
...
```bash
openclaw dynamodb-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw dynamodb-guide | openclaw task-decomposer
```
