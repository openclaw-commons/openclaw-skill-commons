...
```bash
npx clawhub@latest install expo-guide
```
...
# 查看帮助
openclaw expo-guide --help

# 指定配置
openclaw expo-guide --config ./config.yaml
```
...
```bash
openclaw expo-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw expo-guide | openclaw task-decomposer
```
