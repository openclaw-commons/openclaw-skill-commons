...
```bash
npx clawhub@latest install nlp-spacy
```
...
# 查看帮助
openclaw nlp-spacy --help

# 指定配置
openclaw nlp-spacy --config ./config.yaml
```
...
```bash
openclaw nlp-spacy --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nlp-spacy | openclaw task-decomposer
```
