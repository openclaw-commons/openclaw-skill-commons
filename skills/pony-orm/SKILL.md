...
```bash
npx clawhub@latest install pony-orm
```
...
# 查看帮助
openclaw pony-orm --help

# 指定配置
openclaw pony-orm --config ./config.yaml
```
...
```bash
openclaw pony-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pony-orm | openclaw task-decomposer
```
