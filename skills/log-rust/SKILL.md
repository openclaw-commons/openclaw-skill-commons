...
```bash
npx clawhub@latest install log-rust
```
...
# 查看帮助
openclaw log-rust --help

# 指定配置
openclaw log-rust --config ./config.yaml
```
...
```bash
openclaw log-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw log-rust | openclaw task-decomposer
```
