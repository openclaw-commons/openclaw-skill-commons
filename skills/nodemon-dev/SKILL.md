...
```bash
npx clawhub@latest install nodemon-dev
```
...
```bash
# 使用默认配置
openclaw nodemon-dev

# 查看帮助
openclaw nodemon-dev --help

# 指定配置
openclaw nodemon-dev --config ./config.yaml
```
...
```bash
openclaw nodemon-dev --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nodemon-dev | openclaw task-decomposer
```
