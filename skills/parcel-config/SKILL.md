...
```bash
npx clawhub@latest install parcel-config
```
...
```bash
# 使用默认配置
openclaw parcel-config

# 查看帮助
openclaw parcel-config --help

# 指定配置
openclaw parcel-config --config ./config.yaml
```
...
```bash
openclaw parcel-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw parcel-config | openclaw task-decomposer
```
