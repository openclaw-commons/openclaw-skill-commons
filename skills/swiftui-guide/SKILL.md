...
```bash
npx clawhub@latest install swiftui-guide
```
...
```bash
# 使用默认配置
openclaw swiftui-guide

# 查看帮助
openclaw swiftui-guide --help

# 指定配置
openclaw swiftui-guide --config ./config.yaml
```
...
```bash
openclaw swiftui-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw swiftui-guide | openclaw task-decomposer
```
