...
```bash
npx clawhub@latest install walkdir-rust
```
...
```bash
# 使用默认配置
openclaw walkdir-rust

# 查看帮助
openclaw walkdir-rust --help

# 指定配置
openclaw walkdir-rust --config ./config.yaml
```
...
```bash
openclaw walkdir-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw walkdir-rust | openclaw task-decomposer
```
