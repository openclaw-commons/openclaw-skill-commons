...
```bash
npx clawhub@latest install vpn-setup
```
...
# 查看帮助
openclaw vpn-setup --help

# 指定配置
openclaw vpn-setup --config ./config.yaml
```
...
```bash
openclaw vpn-setup --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw vpn-setup | openclaw task-decomposer
```
