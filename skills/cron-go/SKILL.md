...
```bash
npx clawhub@latest install cron-go
```
...
# 查看帮助
openclaw cron-go --help

# 指定配置
openclaw cron-go --config ./config.yaml
```
...
```bash
openclaw cron-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cron-go | openclaw task-decomposer
```
