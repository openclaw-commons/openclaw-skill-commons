...
```bash
npx clawhub@latest install ws-websocket
```
...
# 查看帮助
openclaw ws-websocket --help

# 指定配置
openclaw ws-websocket --config ./config.yaml
```
...
```bash
openclaw ws-websocket --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ws-websocket | openclaw task-decomposer
```
