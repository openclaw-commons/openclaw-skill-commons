...
```bash
npx clawhub@latest install file-sharing
```
...
```bash
# 使用默认配置
openclaw file-sharing

# 查看帮助
openclaw file-sharing --help

# 指定配置
openclaw file-sharing --config ./config.yaml
```
...
```bash
openclaw file-sharing --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw file-sharing | openclaw task-decomposer
```
