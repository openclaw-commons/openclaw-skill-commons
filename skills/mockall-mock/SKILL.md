...
```bash
npx clawhub@latest install mockall-mock
```
...
```bash
# 使用默认配置
openclaw mockall-mock

# 查看帮助
openclaw mockall-mock --help

# 指定配置
openclaw mockall-mock --config ./config.yaml
```
...
```bash
openclaw mockall-mock --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mockall-mock | openclaw task-decomposer
```
