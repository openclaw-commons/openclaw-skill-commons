...
```bash
npx clawhub@latest install hug-api
```
...
# 查看帮助
openclaw hug-api --help

# 指定配置
openclaw hug-api --config ./config.yaml
```
...
```bash
openclaw hug-api --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw hug-api | openclaw task-decomposer
```
