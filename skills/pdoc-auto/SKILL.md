...
```bash
npx clawhub@latest install pdoc-auto
```
...
# 查看帮助
openclaw pdoc-auto --help

# 指定配置
openclaw pdoc-auto --config ./config.yaml
```
...
```bash
openclaw pdoc-auto --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pdoc-auto | openclaw task-decomposer
```
