...
```bash
npx clawhub@latest install rubrik-guide
```
...
```bash
# 使用默认配置
openclaw rubrik-guide

# 查看帮助
openclaw rubrik-guide --help

# 指定配置
openclaw rubrik-guide --config ./config.yaml
```
...
```bash
openclaw rubrik-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rubrik-guide | openclaw task-decomposer
```
