...
```bash
npx clawhub@latest install canvas-node
```
...
```bash
# 使用默认配置
openclaw canvas-node

# 查看帮助
openclaw canvas-node --help

# 指定配置
openclaw canvas-node --config ./config.yaml
```
...
```bash
openclaw canvas-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw canvas-node | openclaw task-decomposer
```
