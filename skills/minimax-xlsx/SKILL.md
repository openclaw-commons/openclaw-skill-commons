...
```bash
npx clawhub@latest install minimax-xlsx
```
...
```bash
# 使用默认配置
openclaw minimax-xlsx

# 查看帮助
openclaw minimax-xlsx --help

# 指定配置
openclaw minimax-xlsx --config ./config.yaml
```
...
```bash
openclaw minimax-xlsx --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw minimax-xlsx | openclaw task-decomposer
```
