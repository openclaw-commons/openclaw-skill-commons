...
```bash
npx clawhub@latest install iced-gui
```
...
# 查看帮助
openclaw iced-gui --help

# 指定配置
openclaw iced-gui --config ./config.yaml
```
...
```bash
openclaw iced-gui --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw iced-gui | openclaw task-decomposer
```
