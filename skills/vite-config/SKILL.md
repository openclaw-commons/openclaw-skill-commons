...
```bash
npx clawhub@latest install vite-config
```
...
```bash
# 使用默认配置
openclaw vite-config

# 查看帮助
openclaw vite-config --help

# 指定配置
openclaw vite-config --config ./config.yaml
```
...
```bash
openclaw vite-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw vite-config | openclaw task-decomposer
```
