...
```bash
npx clawhub@latest install cohesity-guide
```
...
```bash
# 使用默认配置
openclaw cohesity-guide

# 查看帮助
openclaw cohesity-guide --help

# 指定配置
openclaw cohesity-guide --config ./config.yaml
```
...
```bash
openclaw cohesity-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw cohesity-guide | openclaw task-decomposer
```
