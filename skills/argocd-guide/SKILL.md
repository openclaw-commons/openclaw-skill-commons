...
```bash
npx clawhub@latest install argocd-guide
```
...
```bash
# 使用默认配置
openclaw argocd-guide

# 查看帮助
openclaw argocd-guide --help

# 指定配置
openclaw argocd-guide --config ./config.yaml
```
...
```bash
openclaw argocd-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw argocd-guide | openclaw task-decomposer
```
