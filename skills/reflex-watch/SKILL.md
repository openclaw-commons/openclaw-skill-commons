...
```bash
npx clawhub@latest install reflex-watch
```
...
```bash
# 使用默认配置
openclaw reflex-watch

# 查看帮助
openclaw reflex-watch --help

# 指定配置
openclaw reflex-watch --config ./config.yaml
```
...
```bash
openclaw reflex-watch --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw reflex-watch | openclaw task-decomposer
```
