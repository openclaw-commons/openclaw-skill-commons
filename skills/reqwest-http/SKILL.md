...
```bash
npx clawhub@latest install reqwest-http
```
...
```bash
# 使用默认配置
openclaw reqwest-http

# 查看帮助
openclaw reqwest-http --help

# 指定配置
openclaw reqwest-http --config ./config.yaml
```
...
```bash
openclaw reqwest-http --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw reqwest-http | openclaw task-decomposer
```
