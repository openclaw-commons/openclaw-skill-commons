...
```bash
npx clawhub@latest install zap-logging
```
...
```bash
# 使用默认配置
openclaw zap-logging

# 查看帮助
openclaw zap-logging --help

# 指定配置
openclaw zap-logging --config ./config.yaml
```
...
```bash
openclaw zap-logging --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw zap-logging | openclaw task-decomposer
```
