...
```bash
npx clawhub@latest install docopt-cli
```
...
# 查看帮助
openclaw docopt-cli --help

# 指定配置
openclaw docopt-cli --config ./config.yaml
```
...
```bash
openclaw docopt-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw docopt-cli | openclaw task-decomposer
```
