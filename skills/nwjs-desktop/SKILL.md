...
```bash
npx clawhub@latest install nwjs-desktop
```
...
```bash
# 使用默认配置
openclaw nwjs-desktop

# 查看帮助
openclaw nwjs-desktop --help

# 指定配置
openclaw nwjs-desktop --config ./config.yaml
```
...
```bash
openclaw nwjs-desktop --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nwjs-desktop | openclaw task-decomposer
```
