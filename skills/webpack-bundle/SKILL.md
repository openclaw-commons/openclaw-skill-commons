...
> Node.js 开发技能，包含框架和工具

## 📦 安装

```bash
npx clawhub@latest install webpack-bundle
```
...
```bash
# 使用默认配置
openclaw webpack-bundle

# 查看帮助
openclaw webpack-bundle --help

# 指定配置
openclaw webpack-bundle --config ./config.yaml
```
...
```bash
openclaw webpack-bundle --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw webpack-bundle | openclaw task-decomposer
```
