...
```bash
npx clawhub@latest install split-guide
```
...
```bash
# 使用默认配置
openclaw split-guide

# 查看帮助
openclaw split-guide --help

# 指定配置
openclaw split-guide --config ./config.yaml
```
...
```bash
openclaw split-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw split-guide | openclaw task-decomposer
```
