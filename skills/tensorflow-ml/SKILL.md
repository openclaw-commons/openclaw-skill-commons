...
```bash
npx clawhub@latest install tensorflow-ml
```
...
```bash
# 使用默认配置
openclaw tensorflow-ml

# 查看帮助
openclaw tensorflow-ml --help

# 指定配置
openclaw tensorflow-ml --config ./config.yaml
```
...
```bash
openclaw tensorflow-ml --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tensorflow-ml | openclaw task-decomposer
```
