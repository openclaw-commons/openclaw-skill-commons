...
```bash
npx clawhub@latest install env_logger
```
...
# 查看帮助
openclaw env_logger --help

# 指定配置
openclaw env_logger --config ./config.yaml
```
...
```bash
openclaw env_logger --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw env_logger | openclaw task-decomposer
```
