...
```bash
npx clawhub@latest install packaging-pypi
```
...
```bash
# 使用默认配置
openclaw packaging-pypi

# 查看帮助
openclaw packaging-pypi --help

# 指定配置
openclaw packaging-pypi --config ./config.yaml
```
...
```bash
openclaw packaging-pypi --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw packaging-pypi | openclaw task-decomposer
```
