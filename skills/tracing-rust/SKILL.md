...
```bash
npx clawhub@latest install tracing-rust
```
...
```bash
# 使用默认配置
openclaw tracing-rust

# 查看帮助
openclaw tracing-rust --help

# 指定配置
openclaw tracing-rust --config ./config.yaml
```
...
```bash
openclaw tracing-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tracing-rust | openclaw task-decomposer
```
