...
```bash
npx clawhub@latest install convert-guide
```
...
```bash
# 使用默认配置
openclaw convert-guide

# 查看帮助
openclaw convert-guide --help

# 指定配置
openclaw convert-guide --config ./config.yaml
```
...
```bash
openclaw convert-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw convert-guide | openclaw task-decomposer
```
