...
```bash
npx clawhub@latest install bcrypt-hash
```
...
```bash
# 使用默认配置
openclaw bcrypt-hash

# 查看帮助
openclaw bcrypt-hash --help

# 指定配置
openclaw bcrypt-hash --config ./config.yaml
```
...
```bash
openclaw bcrypt-hash --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bcrypt-hash | openclaw task-decomposer
```
