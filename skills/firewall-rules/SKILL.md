...
```bash
npx clawhub@latest install firewall-rules
```
...
```bash
# 使用默认配置
openclaw firewall-rules

# 查看帮助
openclaw firewall-rules --help

# 指定配置
openclaw firewall-rules --config ./config.yaml
```
...
```bash
openclaw firewall-rules --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw firewall-rules | openclaw task-decomposer
```
