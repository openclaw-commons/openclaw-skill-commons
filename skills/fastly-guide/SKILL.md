...
```bash
npx clawhub@latest install fastly-guide
```
...
```bash
# 使用默认配置
openclaw fastly-guide

# 查看帮助
openclaw fastly-guide --help

# 指定配置
openclaw fastly-guide --config ./config.yaml
```
...
```bash
openclaw fastly-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw fastly-guide | openclaw task-decomposer
```
