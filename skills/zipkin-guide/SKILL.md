...
```bash
npx clawhub@latest install zipkin-guide
```
...
```bash
# 使用默认配置
openclaw zipkin-guide

# 查看帮助
openclaw zipkin-guide --help

# 指定配置
openclaw zipkin-guide --config ./config.yaml
```
...
```bash
openclaw zipkin-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw zipkin-guide | openclaw task-decomposer
```
