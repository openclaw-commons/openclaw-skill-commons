...
```bash
npx clawhub@latest install daily-stoic
```
...
```bash
# 使用默认配置
openclaw daily-stoic

# 查看帮助
openclaw daily-stoic --help

# 指定配置
openclaw daily-stoic --config ./config.yaml
```
...
```bash
openclaw daily-stoic --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw daily-stoic | openclaw task-decomposer
```
