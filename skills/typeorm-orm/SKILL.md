...
```bash
npx clawhub@latest install typeorm-orm
```
...
```bash
# 使用默认配置
openclaw typeorm-orm

# 查看帮助
openclaw typeorm-orm --help

# 指定配置
openclaw typeorm-orm --config ./config.yaml
```
...
```bash
openclaw typeorm-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw typeorm-orm | openclaw task-decomposer
```
