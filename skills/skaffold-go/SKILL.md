...
```bash
npx clawhub@latest install skaffold-go
```
...
```bash
# 使用默认配置
openclaw skaffold-go

# 查看帮助
openclaw skaffold-go --help

# 指定配置
openclaw skaffold-go --config ./config.yaml
```
...
```bash
openclaw skaffold-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw skaffold-go | openclaw task-decomposer
```
