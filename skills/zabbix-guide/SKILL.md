...
```bash
npx clawhub@latest install zabbix-guide
```
...
```bash
# 使用默认配置
openclaw zabbix-guide

# 查看帮助
openclaw zabbix-guide --help

# 指定配置
openclaw zabbix-guide --config ./config.yaml
```
...
```bash
openclaw zabbix-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw zabbix-guide | openclaw task-decomposer
```
