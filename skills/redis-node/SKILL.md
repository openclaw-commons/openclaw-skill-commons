...
```bash
npx clawhub@latest install redis-node
```
...
# 查看帮助
openclaw redis-node --help

# 指定配置
openclaw redis-node --config ./config.yaml
```
...
```bash
openclaw redis-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw redis-node | openclaw task-decomposer
```
