...
```bash
npx clawhub@latest install connect-rpc
```
...
```bash
# 使用默认配置
openclaw connect-rpc

# 查看帮助
openclaw connect-rpc --help

# 指定配置
openclaw connect-rpc --config ./config.yaml
```
...
```bash
openclaw connect-rpc --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw connect-rpc | openclaw task-decomposer
```
