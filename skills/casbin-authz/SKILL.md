...
```bash
npx clawhub@latest install casbin-authz
```
...
```bash
# 使用默认配置
openclaw casbin-authz

# 查看帮助
openclaw casbin-authz --help

# 指定配置
openclaw casbin-authz --config ./config.yaml
```
...
```bash
openclaw casbin-authz --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw casbin-authz | openclaw task-decomposer
```
