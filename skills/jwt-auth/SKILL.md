...
```bash
npx clawhub@latest install jwt-auth
```
...
# 查看帮助
openclaw jwt-auth --help

# 指定配置
openclaw jwt-auth --config ./config.yaml
```
...
```bash
openclaw jwt-auth --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw jwt-auth | openclaw task-decomposer
```
