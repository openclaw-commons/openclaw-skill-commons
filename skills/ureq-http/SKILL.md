...
```bash
npx clawhub@latest install ureq-http
```
...
# 查看帮助
openclaw ureq-http --help

# 指定配置
openclaw ureq-http --config ./config.yaml
```
...
```bash
openclaw ureq-http --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ureq-http | openclaw task-decomposer
```
