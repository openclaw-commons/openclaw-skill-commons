...
```bash
npx clawhub@latest install msgpack-node
```
...
```bash
# 使用默认配置
openclaw msgpack-node

# 查看帮助
openclaw msgpack-node --help

# 指定配置
openclaw msgpack-node --config ./config.yaml
```
...
```bash
openclaw msgpack-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw msgpack-node | openclaw task-decomposer
```
