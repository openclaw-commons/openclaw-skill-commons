...
```bash
npx clawhub@latest install changelog-gen
```
...
```bash
# 使用默认配置
openclaw changelog-gen

# 查看帮助
openclaw changelog-gen --help

# 指定配置
openclaw changelog-gen --config ./config.yaml
```
...
```bash
openclaw changelog-gen --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw changelog-gen | openclaw task-decomposer
```
