...
```bash
npx clawhub@latest install rust-crypto
```
...
```bash
# 使用默认配置
openclaw rust-crypto

# 查看帮助
openclaw rust-crypto --help

# 指定配置
openclaw rust-crypto --config ./config.yaml
```
...
```bash
openclaw rust-crypto --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rust-crypto | openclaw task-decomposer
```
