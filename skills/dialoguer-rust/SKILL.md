...
```bash
npx clawhub@latest install dialoguer-rust
```
...
```bash
# 使用默认配置
openclaw dialoguer-rust

# 查看帮助
openclaw dialoguer-rust --help

# 指定配置
openclaw dialoguer-rust --config ./config.yaml
```
...
```bash
openclaw dialoguer-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw dialoguer-rust | openclaw task-decomposer
```
