...
```bash
npx clawhub@latest install responder-web
```
...
```bash
# 使用默认配置
openclaw responder-web

# 查看帮助
openclaw responder-web --help

# 指定配置
openclaw responder-web --config ./config.yaml
```
...
```bash
openclaw responder-web --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw responder-web | openclaw task-decomposer
```
