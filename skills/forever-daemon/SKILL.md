...
```bash
npx clawhub@latest install forever-daemon
```
...
```bash
# 使用默认配置
openclaw forever-daemon

# 查看帮助
openclaw forever-daemon --help

# 指定配置
openclaw forever-daemon --config ./config.yaml
```
...
```bash
openclaw forever-daemon --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw forever-daemon | openclaw task-decomposer
```
