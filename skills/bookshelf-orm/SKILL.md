...
```bash
npx clawhub@latest install bookshelf-orm
```
...
```bash
# 使用默认配置
openclaw bookshelf-orm

# 查看帮助
openclaw bookshelf-orm --help

# 指定配置
openclaw bookshelf-orm --config ./config.yaml
```
...
```bash
openclaw bookshelf-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bookshelf-orm | openclaw task-decomposer
```
