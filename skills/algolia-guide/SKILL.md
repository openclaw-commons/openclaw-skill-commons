...
```bash
npx clawhub@latest install algolia-guide
```
...
```bash
# 使用默认配置
openclaw algolia-guide

# 查看帮助
openclaw algolia-guide --help

# 指定配置
openclaw algolia-guide --config ./config.yaml
```
...
```bash
openclaw algolia-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw algolia-guide | openclaw task-decomposer
```
