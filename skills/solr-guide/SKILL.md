...
```bash
npx clawhub@latest install solr-guide
```
...
# 查看帮助
openclaw solr-guide --help

# 指定配置
openclaw solr-guide --config ./config.yaml
```
...
```bash
openclaw solr-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw solr-guide | openclaw task-decomposer
```
