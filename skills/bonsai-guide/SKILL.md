...
```bash
npx clawhub@latest install bonsai-guide
```
...
```bash
# 使用默认配置
openclaw bonsai-guide

# 查看帮助
openclaw bonsai-guide --help

# 指定配置
openclaw bonsai-guide --config ./config.yaml
```
...
```bash
openclaw bonsai-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bonsai-guide | openclaw task-decomposer
```
