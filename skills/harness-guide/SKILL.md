...
```bash
npx clawhub@latest install harness-guide
```
...
# 查看帮助
openclaw harness-guide --help

# 指定配置
openclaw harness-guide --config ./config.yaml
```
...
```bash
openclaw harness-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw harness-guide | openclaw task-decomposer
```
