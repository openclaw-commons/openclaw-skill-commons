...
```bash
npx clawhub@latest install watcher-go
```
...
# 查看帮助
openclaw watcher-go --help

# 指定配置
openclaw watcher-go --config ./config.yaml
```
...
```bash
openclaw watcher-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw watcher-go | openclaw task-decomposer
```
