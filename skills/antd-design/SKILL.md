...
```bash
npx clawhub@latest install antd-design
```
...
```bash
# 使用默认配置
openclaw antd-design

# 查看帮助
openclaw antd-design --help

# 指定配置
openclaw antd-design --config ./config.yaml
```
...
```bash
openclaw antd-design --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw antd-design | openclaw task-decomposer
```
