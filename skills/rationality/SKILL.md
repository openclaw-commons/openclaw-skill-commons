...
```bash
# 使用默认配置
openclaw rationality

# 查看帮助
openclaw rationality --help

# 指定配置
openclaw rationality --config ./config.yaml
```
...
```bash
openclaw rationality --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rationality | openclaw task-decomposer
```
