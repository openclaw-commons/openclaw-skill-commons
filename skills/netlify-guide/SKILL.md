...
```bash
npx clawhub@latest install netlify-guide
```
...
```bash
# 使用默认配置
openclaw netlify-guide

# 查看帮助
openclaw netlify-guide --help

# 指定配置
openclaw netlify-guide --config ./config.yaml
```
...
```bash
openclaw netlify-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw netlify-guide | openclaw task-decomposer
```
