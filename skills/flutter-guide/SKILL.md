...
```bash
npx clawhub@latest install flutter-guide
```
...
```bash
# 使用默认配置
openclaw flutter-guide

# 查看帮助
openclaw flutter-guide --help

# 指定配置
openclaw flutter-guide --config ./config.yaml
```
...
```bash
openclaw flutter-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw flutter-guide | openclaw task-decomposer
```
