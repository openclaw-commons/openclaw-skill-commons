...
```bash
npx clawhub@latest install snyk-go
```
...
# 查看帮助
openclaw snyk-go --help

# 指定配置
openclaw snyk-go --config ./config.yaml
```
...
```bash
openclaw snyk-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw snyk-go | openclaw task-decomposer
```
