...
```bash
npx clawhub@latest install smol-rust
```
...
# 查看帮助
openclaw smol-rust --help

# 指定配置
openclaw smol-rust --config ./config.yaml
```
...
```bash
openclaw smol-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw smol-rust | openclaw task-decomposer
```
