...
```bash
npx clawhub@latest install ssh-tunnel
```
...
# 查看帮助
openclaw ssh-tunnel --help

# 指定配置
openclaw ssh-tunnel --config ./config.yaml
```
...
```bash
openclaw ssh-tunnel --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ssh-tunnel | openclaw task-decomposer
```
