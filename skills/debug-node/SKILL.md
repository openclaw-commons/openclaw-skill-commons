...
```bash
npx clawhub@latest install debug-node
```
...
# 查看帮助
openclaw debug-node --help

# 指定配置
openclaw debug-node --config ./config.yaml
```
...
```bash
openclaw debug-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw debug-node | openclaw task-decomposer
```
