...
```bash
npx clawhub@latest install flux-cd
```
...
# 查看帮助
openclaw flux-cd --help

# 指定配置
openclaw flux-cd --config ./config.yaml
```
...
```bash
openclaw flux-cd --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw flux-cd | openclaw task-decomposer
```
