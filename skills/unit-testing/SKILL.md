...
```bash
npx clawhub@latest install unit-testing
```
...
```bash
# 使用默认配置
openclaw unit-testing

# 查看帮助
openclaw unit-testing --help

# 指定配置
openclaw unit-testing --config ./config.yaml
```
...
```bash
openclaw unit-testing --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw unit-testing | openclaw task-decomposer
```
