...
```bash
npx clawhub@latest install grpc-node
```
...
# 查看帮助
openclaw grpc-node --help

# 指定配置
openclaw grpc-node --config ./config.yaml
```
...
```bash
openclaw grpc-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw grpc-node | openclaw task-decomposer
```
