...
```bash
npx clawhub@latest install chi-router
```
...
# 查看帮助
openclaw chi-router --help

# 指定配置
openclaw chi-router --config ./config.yaml
```
...
```bash
openclaw chi-router --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw chi-router | openclaw task-decomposer
```
