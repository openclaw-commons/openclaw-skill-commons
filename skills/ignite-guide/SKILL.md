...
```bash
npx clawhub@latest install ignite-guide
```
...
```bash
# 使用默认配置
openclaw ignite-guide

# 查看帮助
openclaw ignite-guide --help

# 指定配置
openclaw ignite-guide --config ./config.yaml
```
...
```bash
openclaw ignite-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ignite-guide | openclaw task-decomposer
```
