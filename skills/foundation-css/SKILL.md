...
```bash
npx clawhub@latest install foundation-css
```
...
```bash
# 使用默认配置
openclaw foundation-css

# 查看帮助
openclaw foundation-css --help

# 指定配置
openclaw foundation-css --config ./config.yaml
```
...
```bash
openclaw foundation-css --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw foundation-css | openclaw task-decomposer
```
