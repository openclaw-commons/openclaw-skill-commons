...
```bash
npx clawhub@latest install native-tls
```
...
# 查看帮助
openclaw native-tls --help

# 指定配置
openclaw native-tls --config ./config.yaml
```
...
```bash
openclaw native-tls --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw native-tls | openclaw task-decomposer
```
