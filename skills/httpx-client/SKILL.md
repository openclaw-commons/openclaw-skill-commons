...
```bash
npx clawhub@latest install httpx-client
```
...
```bash
# 使用默认配置
openclaw httpx-client

# 查看帮助
openclaw httpx-client --help

# 指定配置
openclaw httpx-client --config ./config.yaml
```
...
```bash
openclaw httpx-client --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw httpx-client | openclaw task-decomposer
```
