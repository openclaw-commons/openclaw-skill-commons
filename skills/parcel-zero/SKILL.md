...
```bash
npx clawhub@latest install parcel-zero
```
...
```bash
# 使用默认配置
openclaw parcel-zero

# 查看帮助
openclaw parcel-zero --help

# 指定配置
openclaw parcel-zero --config ./config.yaml
```
...
```bash
openclaw parcel-zero --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw parcel-zero | openclaw task-decomposer
```
