...
```bash
npx clawhub@latest install artemis-guide
```
...
```bash
# 使用默认配置
openclaw artemis-guide

# 查看帮助
openclaw artemis-guide --help

# 指定配置
openclaw artemis-guide --config ./config.yaml
```
...
```bash
openclaw artemis-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw artemis-guide | openclaw task-decomposer
```
