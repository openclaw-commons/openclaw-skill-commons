...
```bash
npx clawhub@latest install warp-rust
```
...
# 查看帮助
openclaw warp-rust --help

# 指定配置
openclaw warp-rust --config ./config.yaml
```
...
```bash
openclaw warp-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw warp-rust | openclaw task-decomposer
```
