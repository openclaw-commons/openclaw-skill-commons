...
```bash
npx clawhub@latest install redis-go
```
...
# 查看帮助
openclaw redis-go --help

# 指定配置
openclaw redis-go --config ./config.yaml
```
...
```bash
openclaw redis-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw redis-go | openclaw task-decomposer
```
