...
```bash
npx clawhub@latest install pytest-django
```
...
```bash
# 使用默认配置
openclaw pytest-django

# 查看帮助
openclaw pytest-django --help

# 指定配置
openclaw pytest-django --config ./config.yaml
```
...
```bash
openclaw pytest-django --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pytest-django | openclaw task-decomposer
```
