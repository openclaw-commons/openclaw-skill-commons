...
```bash
npx clawhub@latest install express-server
```
...
# 查看帮助
openclaw express-server --help

# 指定配置
openclaw express-server --config ./config.yaml
```
...
```bash
openclaw express-server --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw express-server | openclaw task-decomposer
```
