...
```bash
npx clawhub@latest install qa-checklist
```
...
# 查看帮助
openclaw qa-checklist --help

# 指定配置
openclaw qa-checklist --config ./config.yaml
```
...
```bash
openclaw qa-checklist --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw qa-checklist | openclaw task-decomposer
```
