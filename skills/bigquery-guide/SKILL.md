...
```bash
npx clawhub@latest install bigquery-guide
```
...
```bash
# 使用默认配置
openclaw bigquery-guide

# 查看帮助
openclaw bigquery-guide --help

# 指定配置
openclaw bigquery-guide --config ./config.yaml
```
...
```bash
openclaw bigquery-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bigquery-guide | openclaw task-decomposer
```
