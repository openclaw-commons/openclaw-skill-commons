...
```bash
npx clawhub@latest install gcp-go
```
...
# 查看帮助
openclaw gcp-go --help

# 指定配置
openclaw gcp-go --config ./config.yaml
```
...
```bash
openclaw gcp-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gcp-go | openclaw task-decomposer
```
