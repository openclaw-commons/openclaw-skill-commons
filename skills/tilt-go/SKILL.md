...
```bash
npx clawhub@latest install tilt-go
```
...
# 查看帮助
openclaw tilt-go --help

# 指定配置
openclaw tilt-go --config ./config.yaml
```
...
```bash
openclaw tilt-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tilt-go | openclaw task-decomposer
```
