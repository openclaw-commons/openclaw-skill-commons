...
```bash
npx clawhub@latest install splunk-guide
```
...
```bash
# 使用默认配置
openclaw splunk-guide

# 查看帮助
openclaw splunk-guide --help

# 指定配置
openclaw splunk-guide --config ./config.yaml
```
...
```bash
openclaw splunk-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw splunk-guide | openclaw task-decomposer
```
