...
```bash
npx clawhub@latest install ignore-rust
```
...
```bash
# 使用默认配置
openclaw ignore-rust

# 查看帮助
openclaw ignore-rust --help

# 指定配置
openclaw ignore-rust --config ./config.yaml
```
...
```bash
openclaw ignore-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ignore-rust | openclaw task-decomposer
```
