...
```bash
npx clawhub@latest install mysql-guide
```
...
```bash
# 使用默认配置
openclaw mysql-guide

# 查看帮助
openclaw mysql-guide --help

# 指定配置
openclaw mysql-guide --config ./config.yaml
```
...
```bash
openclaw mysql-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mysql-guide | openclaw task-decomposer
```
