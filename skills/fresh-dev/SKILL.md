...
```bash
npx clawhub@latest install fresh-dev
```
...
# 查看帮助
openclaw fresh-dev --help

# 指定配置
openclaw fresh-dev --config ./config.yaml
```
...
```bash
openclaw fresh-dev --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw fresh-dev | openclaw task-decomposer
```
