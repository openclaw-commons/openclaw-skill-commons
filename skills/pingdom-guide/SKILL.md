...
```bash
npx clawhub@latest install pingdom-guide
```
...
```bash
# 使用默认配置
openclaw pingdom-guide

# 查看帮助
openclaw pingdom-guide --help

# 指定配置
openclaw pingdom-guide --config ./config.yaml
```
...
```bash
openclaw pingdom-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pingdom-guide | openclaw task-decomposer
```
