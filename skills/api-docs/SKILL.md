...
```bash
npx clawhub@latest install api-docs
```
...
# 查看帮助
openclaw api-docs --help

# 指定配置
openclaw api-docs --config ./config.yaml
```
...
```bash
openclaw api-docs --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw api-docs | openclaw task-decomposer
```
