...
```bash
npx clawhub@latest install prost-rust
```
...
# 查看帮助
openclaw prost-rust --help

# 指定配置
openclaw prost-rust --config ./config.yaml
```
...
```bash
openclaw prost-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw prost-rust | openclaw task-decomposer
```
