...
```bash
npx clawhub@latest install apex-log
```
...
# 查看帮助
openclaw apex-log --help

# 指定配置
openclaw apex-log --config ./config.yaml
```
...
```bash
openclaw apex-log --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw apex-log | openclaw task-decomposer
```
