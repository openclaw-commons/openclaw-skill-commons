...
```bash
npx clawhub@latest install letsencrypt-go
```
...
```bash
# 使用默认配置
openclaw letsencrypt-go

# 查看帮助
openclaw letsencrypt-go --help

# 指定配置
openclaw letsencrypt-go --config ./config.yaml
```
...
```bash
openclaw letsencrypt-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw letsencrypt-go | openclaw task-decomposer
```
