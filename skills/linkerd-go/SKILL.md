...
```bash
npx clawhub@latest install linkerd-go
```
...
# 查看帮助
openclaw linkerd-go --help

# 指定配置
openclaw linkerd-go --config ./config.yaml
```
...
```bash
openclaw linkerd-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw linkerd-go | openclaw task-decomposer
```
