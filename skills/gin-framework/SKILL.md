...
```bash
npx clawhub@latest install gin-framework
```
...
```bash
# 使用默认配置
openclaw gin-framework

# 查看帮助
openclaw gin-framework --help

# 指定配置
openclaw gin-framework --config ./config.yaml
```
...
```bash
openclaw gin-framework --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gin-framework | openclaw task-decomposer
```
