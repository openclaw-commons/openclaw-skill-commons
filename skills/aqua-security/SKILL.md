...
```bash
npx clawhub@latest install aqua-security
```
...
```bash
# 使用默认配置
openclaw aqua-security

# 查看帮助
openclaw aqua-security --help

# 指定配置
openclaw aqua-security --config ./config.yaml
```
...
```bash
openclaw aqua-security --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw aqua-security | openclaw task-decomposer
```
