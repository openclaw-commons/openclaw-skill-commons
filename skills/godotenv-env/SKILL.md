...
```bash
npx clawhub@latest install godotenv-env
```
...
```bash
# 使用默认配置
openclaw godotenv-env

# 查看帮助
openclaw godotenv-env --help

# 指定配置
openclaw godotenv-env --config ./config.yaml
```
...
```bash
openclaw godotenv-env --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw godotenv-env | openclaw task-decomposer
```
