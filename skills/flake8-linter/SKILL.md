...
```bash
npx clawhub@latest install flake8-linter
```
...
```bash
# 使用默认配置
openclaw flake8-linter

# 查看帮助
openclaw flake8-linter --help

# 指定配置
openclaw flake8-linter --config ./config.yaml
```
...
```bash
openclaw flake8-linter --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw flake8-linter | openclaw task-decomposer
```
