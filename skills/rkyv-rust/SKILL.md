...
```bash
npx clawhub@latest install rkyv-rust
```
...
# 查看帮助
openclaw rkyv-rust --help

# 指定配置
openclaw rkyv-rust --config ./config.yaml
```
...
```bash
openclaw rkyv-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rkyv-rust | openclaw task-decomposer
```
