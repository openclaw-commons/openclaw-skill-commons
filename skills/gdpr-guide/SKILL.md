...
```bash
npx clawhub@latest install gdpr-guide
```
...
# 查看帮助
openclaw gdpr-guide --help

# 指定配置
openclaw gdpr-guide --config ./config.yaml
```
...
```bash
openclaw gdpr-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gdpr-guide | openclaw task-decomposer
```
