...
```bash
npx clawhub@latest install tornado-server
```
...
```bash
# 使用默认配置
openclaw tornado-server

# 查看帮助
openclaw tornado-server --help

# 指定配置
openclaw tornado-server --config ./config.yaml
```
...
```bash
openclaw tornado-server --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tornado-server | openclaw task-decomposer
```
