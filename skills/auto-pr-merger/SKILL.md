...
```bash
# 使用默认配置
openclaw auto-pr-merger

# 查看帮助
openclaw auto-pr-merger --help

# 指定配置
openclaw auto-pr-merger --config ./config.yaml
```
...
```bash
openclaw auto-pr-merger --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw auto-pr-merger | openclaw task-decomposer
```
