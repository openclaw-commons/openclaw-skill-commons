...
```bash
npx clawhub@latest install tailwind-css
```
...
```bash
# 使用默认配置
openclaw tailwind-css

# 查看帮助
openclaw tailwind-css --help

# 指定配置
openclaw tailwind-css --config ./config.yaml
```
...
```bash
openclaw tailwind-css --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tailwind-css | openclaw task-decomposer
```
