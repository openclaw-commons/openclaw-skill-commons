...
```bash
npx clawhub@latest install material-ui
```
...
```bash
# 使用默认配置
openclaw material-ui

# 查看帮助
openclaw material-ui --help

# 指定配置
openclaw material-ui --config ./config.yaml
```
...
```bash
openclaw material-ui --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw material-ui | openclaw task-decomposer
```
