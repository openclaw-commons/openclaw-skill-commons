...
```bash
npx clawhub@latest install taskq-queue
```
...
```bash
# 使用默认配置
openclaw taskq-queue

# 查看帮助
openclaw taskq-queue --help

# 指定配置
openclaw taskq-queue --config ./config.yaml
```
...
```bash
openclaw taskq-queue --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw taskq-queue | openclaw task-decomposer
```
