...
```bash
npx clawhub@latest install nextjs-ssr
```
...
# 查看帮助
openclaw nextjs-ssr --help

# 指定配置
openclaw nextjs-ssr --config ./config.yaml
```
...
```bash
openclaw nextjs-ssr --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nextjs-ssr | openclaw task-decomposer
```
