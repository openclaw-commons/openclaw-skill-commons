...
```bash
npx clawhub@latest install tempfile-rust
```
...
```bash
# 使用默认配置
openclaw tempfile-rust

# 查看帮助
openclaw tempfile-rust --help

# 指定配置
openclaw tempfile-rust --config ./config.yaml
```
...
```bash
openclaw tempfile-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tempfile-rust | openclaw task-decomposer
```
