...
```bash
npx clawhub@latest install rancher-guide
```
...
```bash
# 使用默认配置
openclaw rancher-guide

# 查看帮助
openclaw rancher-guide --help

# 指定配置
openclaw rancher-guide --config ./config.yaml
```
...
```bash
openclaw rancher-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rancher-guide | openclaw task-decomposer
```
