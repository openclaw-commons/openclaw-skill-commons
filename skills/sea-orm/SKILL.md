...
```bash
npx clawhub@latest install sea-orm
```
...
# 查看帮助
openclaw sea-orm --help

# 指定配置
openclaw sea-orm --config ./config.yaml
```
...
```bash
openclaw sea-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sea-orm | openclaw task-decomposer
```
