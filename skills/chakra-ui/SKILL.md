...
```bash
npx clawhub@latest install chakra-ui
```
...
# 查看帮助
openclaw chakra-ui --help

# 指定配置
openclaw chakra-ui --config ./config.yaml
```
...
```bash
openclaw chakra-ui --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw chakra-ui | openclaw task-decomposer
```
