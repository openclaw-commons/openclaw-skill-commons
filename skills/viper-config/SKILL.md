...
```bash
npx clawhub@latest install viper-config
```
...
```bash
# 使用默认配置
openclaw viper-config

# 查看帮助
openclaw viper-config --help

# 指定配置
openclaw viper-config --config ./config.yaml
```
...
```bash
openclaw viper-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw viper-config | openclaw task-decomposer
```
