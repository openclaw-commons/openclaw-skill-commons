...
```bash
npx clawhub@latest install node-monitor
```
...
```bash
# 使用默认配置
openclaw node-monitor

# 查看帮助
openclaw node-monitor --help

# 指定配置
openclaw node-monitor --config ./config.yaml
```
...
```bash
openclaw node-monitor --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw node-monitor | openclaw task-decomposer
```
