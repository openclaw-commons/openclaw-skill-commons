...
```bash
npx clawhub@latest install snowpack-guide
```
...
```bash
# 使用默认配置
openclaw snowpack-guide

# 查看帮助
openclaw snowpack-guide --help

# 指定配置
openclaw snowpack-guide --config ./config.yaml
```
...
```bash
openclaw snowpack-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw snowpack-guide | openclaw task-decomposer
```
