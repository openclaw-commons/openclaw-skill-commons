...
```bash
npx clawhub@latest install celery-tasks
```
...
```bash
# 使用默认配置
openclaw celery-tasks

# 查看帮助
openclaw celery-tasks --help

# 指定配置
openclaw celery-tasks --config ./config.yaml
```
...
```bash
openclaw celery-tasks --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw celery-tasks | openclaw task-decomposer
```
