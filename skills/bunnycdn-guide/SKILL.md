...
```bash
npx clawhub@latest install bunnycdn-guide
```
...
```bash
# 使用默认配置
openclaw bunnycdn-guide

# 查看帮助
openclaw bunnycdn-guide --help

# 指定配置
openclaw bunnycdn-guide --config ./config.yaml
```
...
```bash
openclaw bunnycdn-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bunnycdn-guide | openclaw task-decomposer
```
