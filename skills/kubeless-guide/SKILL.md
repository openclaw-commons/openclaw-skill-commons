...
```bash
npx clawhub@latest install kubeless-guide
```
...
```bash
# 使用默认配置
openclaw kubeless-guide

# 查看帮助
openclaw kubeless-guide --help

# 指定配置
openclaw kubeless-guide --config ./config.yaml
```
...
```bash
openclaw kubeless-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw kubeless-guide | openclaw task-decomposer
```
