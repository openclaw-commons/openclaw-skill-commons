...
```bash
# 使用默认配置
openclaw youtube-data

# 查看帮助
openclaw youtube-data --help

# 指定配置
openclaw youtube-data --config ./config.yaml
```
...
```bash
openclaw youtube-data --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw youtube-data | openclaw task-decomposer
```
