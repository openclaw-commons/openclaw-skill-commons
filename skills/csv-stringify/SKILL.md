...
```bash
npx clawhub@latest install csv-stringify
```
...
```bash
# 使用默认配置
openclaw csv-stringify

# 查看帮助
openclaw csv-stringify --help

# 指定配置
openclaw csv-stringify --config ./config.yaml
```
...
```bash
openclaw csv-stringify --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw csv-stringify | openclaw task-decomposer
```
