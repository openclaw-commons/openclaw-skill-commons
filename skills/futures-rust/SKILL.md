...
```bash
npx clawhub@latest install futures-rust
```
...
```bash
# 使用默认配置
openclaw futures-rust

# 查看帮助
openclaw futures-rust --help

# 指定配置
openclaw futures-rust --config ./config.yaml
```
...
```bash
openclaw futures-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw futures-rust | openclaw task-decomposer
```
