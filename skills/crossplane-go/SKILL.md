...
```bash
npx clawhub@latest install crossplane-go
```
...
```bash
# 使用默认配置
openclaw crossplane-go

# 查看帮助
openclaw crossplane-go --help

# 指定配置
openclaw crossplane-go --config ./config.yaml
```
...
```bash
openclaw crossplane-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw crossplane-go | openclaw task-decomposer
```
