...
```bash
npx clawhub@latest install chai-assertion
```
...
```bash
# 使用默认配置
openclaw chai-assertion

# 查看帮助
openclaw chai-assertion --help

# 指定配置
openclaw chai-assertion --config ./config.yaml
```
...
```bash
openclaw chai-assertion --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw chai-assertion | openclaw task-decomposer
```
