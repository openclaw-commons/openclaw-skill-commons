...
```bash
npx clawhub@latest install dependabot-go
```
...
```bash
# 使用默认配置
openclaw dependabot-go

# 查看帮助
openclaw dependabot-go --help

# 指定配置
openclaw dependabot-go --config ./config.yaml
```
...
```bash
openclaw dependabot-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw dependabot-go | openclaw task-decomposer
```
