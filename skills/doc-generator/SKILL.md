...
```bash
npx clawhub@latest install doc-generator
```
...
# 查看帮助
openclaw doc-generator --help

# 指定配置
openclaw doc-generator --config ./config.yaml
```
...
```bash
openclaw doc-generator --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw doc-generator | openclaw task-decomposer
```
