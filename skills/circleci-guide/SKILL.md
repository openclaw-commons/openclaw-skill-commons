...
```bash
npx clawhub@latest install circleci-guide
```
...
```bash
# 使用默认配置
openclaw circleci-guide

# 查看帮助
openclaw circleci-guide --help

# 指定配置
openclaw circleci-guide --config ./config.yaml
```
...
```bash
openclaw circleci-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw circleci-guide | openclaw task-decomposer
```
