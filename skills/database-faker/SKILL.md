...
```bash
npx clawhub@latest install database-faker
```
...
```bash
# 使用默认配置
openclaw database-faker

# 查看帮助
openclaw database-faker --help

# 指定配置
openclaw database-faker --config ./config.yaml
```
...
```bash
openclaw database-faker --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw database-faker | openclaw task-decomposer
```
