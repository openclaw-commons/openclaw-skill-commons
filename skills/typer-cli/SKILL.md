...
```bash
npx clawhub@latest install typer-cli
```
...
# 查看帮助
openclaw typer-cli --help

# 指定配置
openclaw typer-cli --config ./config.yaml
```
...
```bash
openclaw typer-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw typer-cli | openclaw task-decomposer
```
