...
```bash
npx clawhub@latest install macroquad-game
```
...
```bash
# 使用默认配置
openclaw macroquad-game

# 查看帮助
openclaw macroquad-game --help

# 指定配置
openclaw macroquad-game --config ./config.yaml
```
...
```bash
openclaw macroquad-game --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw macroquad-game | openclaw task-decomposer
```
