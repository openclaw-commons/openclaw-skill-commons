...
```bash
npx clawhub@latest install grep-cli
```
...
# 查看帮助
openclaw grep-cli --help

# 指定配置
openclaw grep-cli --config ./config.yaml
```
...
```bash
openclaw grep-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw grep-cli | openclaw task-decomposer
```
