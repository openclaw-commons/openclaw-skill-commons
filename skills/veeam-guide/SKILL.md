...
```bash
npx clawhub@latest install veeam-guide
```
...
```bash
# 使用默认配置
openclaw veeam-guide

# 查看帮助
openclaw veeam-guide --help

# 指定配置
openclaw veeam-guide --config ./config.yaml
```
...
```bash
openclaw veeam-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw veeam-guide | openclaw task-decomposer
```
