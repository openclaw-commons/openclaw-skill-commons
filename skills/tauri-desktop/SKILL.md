...
```bash
npx clawhub@latest install tauri-desktop
```
...
```bash
# 使用默认配置
openclaw tauri-desktop

# 查看帮助
openclaw tauri-desktop --help

# 指定配置
openclaw tauri-desktop --config ./config.yaml
```
...
```bash
openclaw tauri-desktop --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tauri-desktop | openclaw task-decomposer
```
