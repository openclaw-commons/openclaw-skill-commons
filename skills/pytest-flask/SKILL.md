...
```bash
npx clawhub@latest install pytest-flask
```
...
```bash
# 使用默认配置
openclaw pytest-flask

# 查看帮助
openclaw pytest-flask --help

# 指定配置
openclaw pytest-flask --config ./config.yaml
```
...
```bash
openclaw pytest-flask --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pytest-flask | openclaw task-decomposer
```
