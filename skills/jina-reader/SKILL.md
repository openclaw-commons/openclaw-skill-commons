...
```bash
# 使用默认配置
openclaw jina-reader

# 查看帮助
openclaw jina-reader --help

# 指定配置
openclaw jina-reader --config ./config.yaml
```
...
```bash
openclaw jina-reader --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw jina-reader | openclaw task-decomposer
```
