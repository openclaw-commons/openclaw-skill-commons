...
```bash
npx clawhub@latest install mock-server
```
...
# 查看帮助
openclaw mock-server --help

# 指定配置
openclaw mock-server --config ./config.yaml
```
...
```bash
openclaw mock-server --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mock-server | openclaw task-decomposer
```
