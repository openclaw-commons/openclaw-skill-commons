...
```bash
npx clawhub@latest install lazy-regex
```
...
# 查看帮助
openclaw lazy-regex --help

# 指定配置
openclaw lazy-regex --config ./config.yaml
```
...
```bash
openclaw lazy-regex --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw lazy-regex | openclaw task-decomposer
```
