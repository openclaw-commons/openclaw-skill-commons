...
```bash
npx clawhub@latest install ginkgo-bdd
```
...
# 查看帮助
openclaw ginkgo-bdd --help

# 指定配置
openclaw ginkgo-bdd --config ./config.yaml
```
...
```bash
openclaw ginkgo-bdd --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ginkgo-bdd | openclaw task-decomposer
```
