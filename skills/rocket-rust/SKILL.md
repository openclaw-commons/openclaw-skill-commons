...
```bash
npx clawhub@latest install rocket-rust
```
...
```bash
# 使用默认配置
openclaw rocket-rust

# 查看帮助
openclaw rocket-rust --help

# 指定配置
openclaw rocket-rust --config ./config.yaml
```
...
```bash
openclaw rocket-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw rocket-rust | openclaw task-decomposer
```
