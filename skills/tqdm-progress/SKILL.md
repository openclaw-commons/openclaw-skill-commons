...
```bash
npx clawhub@latest install tqdm-progress
```
...
```bash
# 使用默认配置
openclaw tqdm-progress

# 查看帮助
openclaw tqdm-progress --help

# 指定配置
openclaw tqdm-progress --config ./config.yaml
```
...
```bash
openclaw tqdm-progress --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tqdm-progress | openclaw task-decomposer
```
