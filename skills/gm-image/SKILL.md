...
```bash
npx clawhub@latest install gm-image
```
...
# 查看帮助
openclaw gm-image --help

# 指定配置
openclaw gm-image --config ./config.yaml
```
...
```bash
openclaw gm-image --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gm-image | openclaw task-decomposer
```
