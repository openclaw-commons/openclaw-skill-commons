...
```bash
npx clawhub@latest install failure-rust
```
...
```bash
# 使用默认配置
openclaw failure-rust

# 查看帮助
openclaw failure-rust --help

# 指定配置
openclaw failure-rust --config ./config.yaml
```
...
```bash
openclaw failure-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw failure-rust | openclaw task-decomposer
```
