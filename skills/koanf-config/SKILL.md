...
```bash
npx clawhub@latest install koanf-config
```
...
```bash
# 使用默认配置
openclaw koanf-config

# 查看帮助
openclaw koanf-config --help

# 指定配置
openclaw koanf-config --config ./config.yaml
```
...
```bash
openclaw koanf-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw koanf-config | openclaw task-decomposer
```
