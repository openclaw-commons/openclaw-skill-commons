...
```bash
npx clawhub@latest install zeromq-guide
```
...
```bash
# 使用默认配置
openclaw zeromq-guide

# 查看帮助
openclaw zeromq-guide --help

# 指定配置
openclaw zeromq-guide --config ./config.yaml
```
...
```bash
openclaw zeromq-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw zeromq-guide | openclaw task-decomposer
```
