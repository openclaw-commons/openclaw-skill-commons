...
```bash
npx clawhub@latest install gorm-orm
```
...
# 查看帮助
openclaw gorm-orm --help

# 指定配置
openclaw gorm-orm --config ./config.yaml
```
...
```bash
openclaw gorm-orm --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gorm-orm | openclaw task-decomposer
```
