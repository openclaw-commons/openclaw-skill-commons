...
```bash
npx clawhub@latest install yarn-workspace
```
...
```bash
# 使用默认配置
openclaw yarn-workspace

# 查看帮助
openclaw yarn-workspace --help

# 指定配置
openclaw yarn-workspace --config ./config.yaml
```
...
```bash
openclaw yarn-workspace --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw yarn-workspace | openclaw task-decomposer
```
