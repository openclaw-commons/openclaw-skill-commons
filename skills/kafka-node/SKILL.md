...
```bash
npx clawhub@latest install kafka-node
```
...
# 查看帮助
openclaw kafka-node --help

# 指定配置
openclaw kafka-node --config ./config.yaml
```
...
```bash
openclaw kafka-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw kafka-node | openclaw task-decomposer
```
