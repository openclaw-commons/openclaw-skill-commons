...
```bash
npx clawhub@latest install owasp-guide
```
...
```bash
# 使用默认配置
openclaw owasp-guide

# 查看帮助
openclaw owasp-guide --help

# 指定配置
openclaw owasp-guide --config ./config.yaml
```
...
```bash
openclaw owasp-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw owasp-guide | openclaw task-decomposer
```
