...
```bash
npx clawhub@latest install sanic-async
```
...
```bash
# 使用默认配置
openclaw sanic-async

# 查看帮助
openclaw sanic-async --help

# 指定配置
openclaw sanic-async --config ./config.yaml
```
...
```bash
openclaw sanic-async --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sanic-async | openclaw task-decomposer
```
