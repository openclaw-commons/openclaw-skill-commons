...
```bash
npx clawhub@latest install aiohttp-async
```
...
```bash
# 使用默认配置
openclaw aiohttp-async

# 查看帮助
openclaw aiohttp-async --help

# 指定配置
openclaw aiohttp-async --config ./config.yaml
```
...
```bash
openclaw aiohttp-async --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw aiohttp-async | openclaw task-decomposer
```
