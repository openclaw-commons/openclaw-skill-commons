...
```bash
npx clawhub@latest install ring-crypto
```
...
```bash
# 使用默认配置
openclaw ring-crypto

# 查看帮助
openclaw ring-crypto --help

# 指定配置
openclaw ring-crypto --config ./config.yaml
```
...
```bash
openclaw ring-crypto --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ring-crypto | openclaw task-decomposer
```
