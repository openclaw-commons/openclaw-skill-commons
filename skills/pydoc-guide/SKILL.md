...
```bash
npx clawhub@latest install pydoc-guide
```
...
```bash
# 使用默认配置
openclaw pydoc-guide

# 查看帮助
openclaw pydoc-guide --help

# 指定配置
openclaw pydoc-guide --config ./config.yaml
```
...
```bash
openclaw pydoc-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pydoc-guide | openclaw task-decomposer
```
