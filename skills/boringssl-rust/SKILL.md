...
```bash
npx clawhub@latest install boringssl-rust
```
...
```bash
# 使用默认配置
openclaw boringssl-rust

# 查看帮助
openclaw boringssl-rust --help

# 指定配置
openclaw boringssl-rust --config ./config.yaml
```
...
```bash
openclaw boringssl-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw boringssl-rust | openclaw task-decomposer
```
