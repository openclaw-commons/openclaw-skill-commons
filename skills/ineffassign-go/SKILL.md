...
```bash
npx clawhub@latest install ineffassign-go
```
...
```bash
# 使用默认配置
openclaw ineffassign-go

# 查看帮助
openclaw ineffassign-go --help

# 指定配置
openclaw ineffassign-go --config ./config.yaml
```
...
```bash
openclaw ineffassign-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ineffassign-go | openclaw task-decomposer
```
