...
```bash
npx clawhub@latest install youtube-search
```
...
```bash
# 使用默认配置
openclaw youtube-search

# 查看帮助
openclaw youtube-search --help

# 指定配置
openclaw youtube-search --config ./config.yaml
```
...
```bash
openclaw youtube-search --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw youtube-search | openclaw task-decomposer
```
