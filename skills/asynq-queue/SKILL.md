...
```bash
npx clawhub@latest install asynq-queue
```
...
```bash
# 使用默认配置
openclaw asynq-queue

# 查看帮助
openclaw asynq-queue --help

# 指定配置
openclaw asynq-queue --config ./config.yaml
```
...
```bash
openclaw asynq-queue --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw asynq-queue | openclaw task-decomposer
```
