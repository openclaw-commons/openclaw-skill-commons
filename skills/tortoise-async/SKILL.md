...
```bash
npx clawhub@latest install tortoise-async
```
...
```bash
# 使用默认配置
openclaw tortoise-async

# 查看帮助
openclaw tortoise-async --help

# 指定配置
openclaw tortoise-async --config ./config.yaml
```
...
```bash
openclaw tortoise-async --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tortoise-async | openclaw task-decomposer
```
