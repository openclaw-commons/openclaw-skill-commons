...
```bash
npx clawhub@latest install nerdctl-go
```
...
# 查看帮助
openclaw nerdctl-go --help

# 指定配置
openclaw nerdctl-go --config ./config.yaml
```
...
```bash
openclaw nerdctl-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nerdctl-go | openclaw task-decomposer
```
