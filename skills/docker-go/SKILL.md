...
```bash
npx clawhub@latest install docker-go
```
...
# 查看帮助
openclaw docker-go --help

# 指定配置
openclaw docker-go --config ./config.yaml
```
...
```bash
openclaw docker-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw docker-go | openclaw task-decomposer
```
