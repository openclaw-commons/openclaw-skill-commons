...
```bash
npx clawhub@latest install echo-framework
```
...
```bash
# 使用默认配置
openclaw echo-framework

# 查看帮助
openclaw echo-framework --help

# 指定配置
openclaw echo-framework --config ./config.yaml
```
...
```bash
openclaw echo-framework --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw echo-framework | openclaw task-decomposer
```
