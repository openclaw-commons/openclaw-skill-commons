...
```bash
npx clawhub@latest install consul-go
```
...
# 查看帮助
openclaw consul-go --help

# 指定配置
openclaw consul-go --config ./config.yaml
```
...
```bash
openclaw consul-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw consul-go | openclaw task-decomposer
```
