...
```bash
npx clawhub@latest install nats-advanced
```
...
```bash
# 使用默认配置
openclaw nats-advanced

# 查看帮助
openclaw nats-advanced --help

# 指定配置
openclaw nats-advanced --config ./config.yaml
```
...
```bash
openclaw nats-advanced --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nats-advanced | openclaw task-decomposer
```
