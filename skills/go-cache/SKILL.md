...
```bash
npx clawhub@latest install go-cache
```
...
# 查看帮助
openclaw go-cache --help

# 指定配置
openclaw go-cache --config ./config.yaml
```
...
```bash
openclaw go-cache --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw go-cache | openclaw task-decomposer
```
