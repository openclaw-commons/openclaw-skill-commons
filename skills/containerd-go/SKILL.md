...
```bash
npx clawhub@latest install containerd-go
```
...
```bash
# 使用默认配置
openclaw containerd-go

# 查看帮助
openclaw containerd-go --help

# 指定配置
openclaw containerd-go --config ./config.yaml
```
...
```bash
openclaw containerd-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw containerd-go | openclaw task-decomposer
```
