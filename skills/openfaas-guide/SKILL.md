...
```bash
npx clawhub@latest install openfaas-guide
```
...
```bash
# 使用默认配置
openclaw openfaas-guide

# 查看帮助
openclaw openfaas-guide --help

# 指定配置
openclaw openfaas-guide --config ./config.yaml
```
...
```bash
openclaw openfaas-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw openfaas-guide | openclaw task-decomposer
```
