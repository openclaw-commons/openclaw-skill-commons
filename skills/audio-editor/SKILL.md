...
```bash
npx clawhub@latest install audio-editor
```
...
```bash
# 使用默认配置
openclaw audio-editor

# 查看帮助
openclaw audio-editor --help

# 指定配置
openclaw audio-editor --config ./config.yaml
```
...
```bash
openclaw audio-editor --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw audio-editor | openclaw task-decomposer
```
