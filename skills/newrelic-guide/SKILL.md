...
```bash
npx clawhub@latest install newrelic-guide
```
...
```bash
# 使用默认配置
openclaw newrelic-guide

# 查看帮助
openclaw newrelic-guide --help

# 指定配置
openclaw newrelic-guide --config ./config.yaml
```
...
```bash
openclaw newrelic-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw newrelic-guide | openclaw task-decomposer
```
