...
```bash
npx clawhub@latest install alive-progress
```
...
```bash
# 使用默认配置
openclaw alive-progress

# 查看帮助
openclaw alive-progress --help

# 指定配置
openclaw alive-progress --config ./config.yaml
```
...
```bash
openclaw alive-progress --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw alive-progress | openclaw task-decomposer
```
