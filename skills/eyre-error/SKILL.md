...
```bash
npx clawhub@latest install eyre-error
```
...
# 查看帮助
openclaw eyre-error --help

# 指定配置
openclaw eyre-error --config ./config.yaml
```
...
```bash
openclaw eyre-error --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw eyre-error | openclaw task-decomposer
```
