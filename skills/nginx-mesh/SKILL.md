...
```bash
npx clawhub@latest install nginx-mesh
```
...
# 查看帮助
openclaw nginx-mesh --help

# 指定配置
openclaw nginx-mesh --config ./config.yaml
```
...
```bash
openclaw nginx-mesh --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nginx-mesh | openclaw task-decomposer
```
