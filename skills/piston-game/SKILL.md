...
```bash
npx clawhub@latest install piston-game
```
...
```bash
# 使用默认配置
openclaw piston-game

# 查看帮助
openclaw piston-game --help

# 指定配置
openclaw piston-game --config ./config.yaml
```
...
```bash
openclaw piston-game --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw piston-game | openclaw task-decomposer
```
