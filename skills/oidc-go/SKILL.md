...
```bash
npx clawhub@latest install oidc-go
```
...
# 查看帮助
openclaw oidc-go --help

# 指定配置
openclaw oidc-go --config ./config.yaml
```
...
```bash
openclaw oidc-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw oidc-go | openclaw task-decomposer
```
