...
```bash
npx clawhub@latest install note-organizer
```
...
```bash
# 使用默认配置
openclaw note-organizer

# 查看帮助
openclaw note-organizer --help

# 指定配置
openclaw note-organizer --config ./config.yaml
```
...
```bash
openclaw note-organizer --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw note-organizer | openclaw task-decomposer
```
