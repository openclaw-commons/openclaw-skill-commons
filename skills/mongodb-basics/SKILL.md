...
```bash
npx clawhub@latest install mongodb-basics
```
...
```bash
# 使用默认配置
openclaw mongodb-basics

# 查看帮助
openclaw mongodb-basics --help

# 指定配置
openclaw mongodb-basics --config ./config.yaml
```
...
```bash
openclaw mongodb-basics --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mongodb-basics | openclaw task-decomposer
```
