...
```bash
npx clawhub@latest install axios-http
```
...
# 查看帮助
openclaw axios-http --help

# 指定配置
openclaw axios-http --config ./config.yaml
```
...
```bash
openclaw axios-http --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw axios-http | openclaw task-decomposer
```
