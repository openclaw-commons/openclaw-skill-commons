...
```bash
npx clawhub@latest install neon-node
```
...
# 查看帮助
openclaw neon-node --help

# 指定配置
openclaw neon-node --config ./config.yaml
```
...
```bash
openclaw neon-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw neon-node | openclaw task-decomposer
```
