...
```bash
npx clawhub@latest install grafana-go
```
...
# 查看帮助
openclaw grafana-go --help

# 指定配置
openclaw grafana-go --config ./config.yaml
```
...
```bash
openclaw grafana-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw grafana-go | openclaw task-decomposer
```
