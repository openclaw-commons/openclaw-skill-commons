...
```bash
npx clawhub@latest install pdf-lib
```
...
# 查看帮助
openclaw pdf-lib --help

# 指定配置
openclaw pdf-lib --config ./config.yaml
```
...
```bash
openclaw pdf-lib --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pdf-lib | openclaw task-decomposer
```
