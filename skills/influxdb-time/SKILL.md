...
```bash
npx clawhub@latest install influxdb-time
```
...
```bash
# 使用默认配置
openclaw influxdb-time

# 查看帮助
openclaw influxdb-time --help

# 指定配置
openclaw influxdb-time --config ./config.yaml
```
...
```bash
openclaw influxdb-time --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw influxdb-time | openclaw task-decomposer
```
