...
```bash
# 使用默认配置
openclaw git-essentials

# 查看帮助
openclaw git-essentials --help

# 指定配置
openclaw git-essentials --config ./config.yaml
```
...
```bash
openclaw git-essentials --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw git-essentials | openclaw task-decomposer
```
