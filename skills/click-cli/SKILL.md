...
```bash
npx clawhub@latest install click-cli
```
...
# 查看帮助
openclaw click-cli --help

# 指定配置
openclaw click-cli --config ./config.yaml
```
...
```bash
openclaw click-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw click-cli | openclaw task-decomposer
```
