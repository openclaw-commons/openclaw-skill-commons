...
```bash
npx clawhub@latest install responses-mock
```
...
```bash
# 使用默认配置
openclaw responses-mock

# 查看帮助
openclaw responses-mock --help

# 指定配置
openclaw responses-mock --config ./config.yaml
```
...
```bash
openclaw responses-mock --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw responses-mock | openclaw task-decomposer
```
