...
```bash
npx clawhub@latest install async-std
```
...
# 查看帮助
openclaw async-std --help

# 指定配置
openclaw async-std --config ./config.yaml
```
...
```bash
openclaw async-std --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw async-std | openclaw task-decomposer
```
