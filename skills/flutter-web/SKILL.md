...
```bash
npx clawhub@latest install flutter-web
```
...
```bash
# 使用默认配置
openclaw flutter-web

# 查看帮助
openclaw flutter-web --help

# 指定配置
openclaw flutter-web --config ./config.yaml
```
...
```bash
openclaw flutter-web --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw flutter-web | openclaw task-decomposer
```
