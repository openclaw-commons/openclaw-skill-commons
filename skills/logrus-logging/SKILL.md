...
```bash
npx clawhub@latest install logrus-logging
```
...
```bash
# 使用默认配置
openclaw logrus-logging

# 查看帮助
openclaw logrus-logging --help

# 指定配置
openclaw logrus-logging --config ./config.yaml
```
...
```bash
openclaw logrus-logging --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw logrus-logging | openclaw task-decomposer
```
