...
```bash
npx clawhub@latest install azure-go
```
...
# 查看帮助
openclaw azure-go --help

# 指定配置
openclaw azure-go --config ./config.yaml
```
...
```bash
openclaw azure-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw azure-go | openclaw task-decomposer
```
