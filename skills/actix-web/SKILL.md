...
```bash
npx clawhub@latest install actix-web
```
...
# 查看帮助
openclaw actix-web --help

# 指定配置
openclaw actix-web --config ./config.yaml
```
...
```bash
openclaw actix-web --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw actix-web | openclaw task-decomposer
```
