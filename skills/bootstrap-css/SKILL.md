...
```bash
npx clawhub@latest install bootstrap-css
```
...
```bash
# 使用默认配置
openclaw bootstrap-css

# 查看帮助
openclaw bootstrap-css --help

# 指定配置
openclaw bootstrap-css --config ./config.yaml
```
...
```bash
openclaw bootstrap-css --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bootstrap-css | openclaw task-decomposer
```
