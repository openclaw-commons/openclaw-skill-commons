...
```bash
npx clawhub@latest install quickcheck
```
...
# 查看帮助
openclaw quickcheck --help

# 指定配置
openclaw quickcheck --config ./config.yaml
```
...
```bash
openclaw quickcheck --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw quickcheck | openclaw task-decomposer
```
