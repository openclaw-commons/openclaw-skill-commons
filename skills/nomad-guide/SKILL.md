...
```bash
npx clawhub@latest install nomad-guide
```
...
```bash
# 使用默认配置
openclaw nomad-guide

# 查看帮助
openclaw nomad-guide --help

# 指定配置
openclaw nomad-guide --config ./config.yaml
```
...
```bash
openclaw nomad-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nomad-guide | openclaw task-decomposer
```
