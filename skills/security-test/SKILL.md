...
```bash
npx clawhub@latest install security-test
```
...
# 查看帮助
openclaw security-test --help

# 指定配置
openclaw security-test --config ./config.yaml
```
...
```bash
openclaw security-test --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw security-test | openclaw task-decomposer
```
