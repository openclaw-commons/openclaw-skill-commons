...
```bash
npx clawhub@latest install fastapi-auth
```
...
```bash
# 使用默认配置
openclaw fastapi-auth

# 查看帮助
openclaw fastapi-auth --help

# 指定配置
openclaw fastapi-auth --config ./config.yaml
```
...
```bash
openclaw fastapi-auth --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw fastapi-auth | openclaw task-decomposer
```
