...
```bash
npx clawhub@latest install wasm-pack
```
...
# 查看帮助
openclaw wasm-pack --help

# 指定配置
openclaw wasm-pack --config ./config.yaml
```
...
```bash
openclaw wasm-pack --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw wasm-pack | openclaw task-decomposer
```
