...
```bash
npx clawhub@latest install bincode-rust
```
...
```bash
# 使用默认配置
openclaw bincode-rust

# 查看帮助
openclaw bincode-rust --help

# 指定配置
openclaw bincode-rust --config ./config.yaml
```
...
```bash
openclaw bincode-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bincode-rust | openclaw task-decomposer
```
