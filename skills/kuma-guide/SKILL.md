...
```bash
npx clawhub@latest install kuma-guide
```
...
# 查看帮助
openclaw kuma-guide --help

# 指定配置
openclaw kuma-guide --config ./config.yaml
```
...
```bash
openclaw kuma-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw kuma-guide | openclaw task-decomposer
```
