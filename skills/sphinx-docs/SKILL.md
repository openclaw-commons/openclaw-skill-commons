...
```bash
npx clawhub@latest install sphinx-docs
```
...
```bash
# 使用默认配置
openclaw sphinx-docs

# 查看帮助
openclaw sphinx-docs --help

# 指定配置
openclaw sphinx-docs --config ./config.yaml
```
...
```bash
openclaw sphinx-docs --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sphinx-docs | openclaw task-decomposer
```
