...
```bash
npx clawhub@latest install sox-compliance
```
...
```bash
# 使用默认配置
openclaw sox-compliance

# 查看帮助
openclaw sox-compliance --help

# 指定配置
openclaw sox-compliance --config ./config.yaml
```
...
```bash
openclaw sox-compliance --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sox-compliance | openclaw task-decomposer
```
