...
```bash
npx clawhub@latest install luigi-workflow
```
...
```bash
# 使用默认配置
openclaw luigi-workflow

# 查看帮助
openclaw luigi-workflow --help

# 指定配置
openclaw luigi-workflow --config ./config.yaml
```
...
```bash
openclaw luigi-workflow --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw luigi-workflow | openclaw task-decomposer
```
