...
```bash
npx clawhub@latest install nsq-go
```
...
# 查看帮助
openclaw nsq-go --help

# 指定配置
openclaw nsq-go --config ./config.yaml
```
...
```bash
openclaw nsq-go --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nsq-go | openclaw task-decomposer
```
