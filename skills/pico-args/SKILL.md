...
```bash
npx clawhub@latest install pico-args
```
...
# 查看帮助
openclaw pico-args --help

# 指定配置
openclaw pico-args --config ./config.yaml
```
...
```bash
openclaw pico-args --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pico-args | openclaw task-decomposer
```
