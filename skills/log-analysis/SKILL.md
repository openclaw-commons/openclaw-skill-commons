...
```bash
# 使用默认配置
openclaw log-analysis

# 查看帮助
openclaw log-analysis --help

# 指定配置
openclaw log-analysis --config ./config.yaml
```
...
```bash
openclaw log-analysis --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw log-analysis | openclaw task-decomposer
```
