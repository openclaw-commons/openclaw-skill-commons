...
```bash
npx clawhub@latest install thrift-node
```
...
```bash
# 使用默认配置
openclaw thrift-node

# 查看帮助
openclaw thrift-node --help

# 指定配置
openclaw thrift-node --config ./config.yaml
```
...
```bash
openclaw thrift-node --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw thrift-node | openclaw task-decomposer
```
