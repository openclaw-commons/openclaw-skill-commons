...
```bash
npx clawhub@latest install just-rust
```
...
# 查看帮助
openclaw just-rust --help

# 指定配置
openclaw just-rust --config ./config.yaml
```
...
```bash
openclaw just-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw just-rust | openclaw task-decomposer
```
