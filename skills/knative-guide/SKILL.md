...
```bash
npx clawhub@latest install knative-guide
```
...
```bash
# 使用默认配置
openclaw knative-guide

# 查看帮助
openclaw knative-guide --help

# 指定配置
openclaw knative-guide --config ./config.yaml
```
...
```bash
openclaw knative-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw knative-guide | openclaw task-decomposer
```
