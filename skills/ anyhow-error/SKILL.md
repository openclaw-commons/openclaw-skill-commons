...
```bash
npx clawhub@latest install  anyhow-error
```
...
```bash
# 使用默认配置
openclaw  anyhow-error

# 查看帮助
openclaw  anyhow-error --help

# 指定配置
openclaw  anyhow-error --config ./config.yaml
```
...
```bash
openclaw  anyhow-error --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw  anyhow-error | openclaw task-decomposer
```
