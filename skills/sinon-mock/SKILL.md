...
```bash
npx clawhub@latest install sinon-mock
```
...
# 查看帮助
openclaw sinon-mock --help

# 指定配置
openclaw sinon-mock --config ./config.yaml
```
...
```bash
openclaw sinon-mock --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw sinon-mock | openclaw task-decomposer
```
