...
```bash
npx clawhub@latest install vcr-cassette
```
...
```bash
# 使用默认配置
openclaw vcr-cassette

# 查看帮助
openclaw vcr-cassette --help

# 指定配置
openclaw vcr-cassette --config ./config.yaml
```
...
```bash
openclaw vcr-cassette --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw vcr-cassette | openclaw task-decomposer
```
