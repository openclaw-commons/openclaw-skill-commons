...
```bash
npx clawhub@latest install kong-guide
```
...
# 查看帮助
openclaw kong-guide --help

# 指定配置
openclaw kong-guide --config ./config.yaml
```
...
```bash
openclaw kong-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw kong-guide | openclaw task-decomposer
```
