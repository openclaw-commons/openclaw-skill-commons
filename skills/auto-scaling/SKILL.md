...
```bash
npx clawhub@latest install auto-scaling
```
...
```bash
# 使用默认配置
openclaw auto-scaling

# 查看帮助
openclaw auto-scaling --help

# 指定配置
openclaw auto-scaling --config ./config.yaml
```
...
```bash
openclaw auto-scaling --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw auto-scaling | openclaw task-decomposer
```
