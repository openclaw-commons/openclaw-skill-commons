...
```bash
npx clawhub@latest install ggez-game
```
...
# 查看帮助
openclaw ggez-game --help

# 指定配置
openclaw ggez-game --config ./config.yaml
```
...
```bash
openclaw ggez-game --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw ggez-game | openclaw task-decomposer
```
