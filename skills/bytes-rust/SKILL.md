...
```bash
npx clawhub@latest install bytes-rust
```
...
# 查看帮助
openclaw bytes-rust --help

# 指定配置
openclaw bytes-rust --config ./config.yaml
```
...
```bash
openclaw bytes-rust --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw bytes-rust | openclaw task-decomposer
```
