...
```bash
npx clawhub@latest install commander-cli
```
...
```bash
# 使用默认配置
openclaw commander-cli

# 查看帮助
openclaw commander-cli --help

# 指定配置
openclaw commander-cli --config ./config.yaml
```
...
```bash
openclaw commander-cli --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw commander-cli | openclaw task-decomposer
```
