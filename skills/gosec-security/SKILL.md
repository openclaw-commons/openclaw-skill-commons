...
```bash
npx clawhub@latest install gosec-security
```
...
```bash
# 使用默认配置
openclaw gosec-security

# 查看帮助
openclaw gosec-security --help

# 指定配置
openclaw gosec-security --config ./config.yaml
```
...
```bash
openclaw gosec-security --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw gosec-security | openclaw task-decomposer
```
