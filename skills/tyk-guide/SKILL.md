...
```bash
npx clawhub@latest install tyk-guide
```
...
# 查看帮助
openclaw tyk-guide --help

# 指定配置
openclaw tyk-guide --config ./config.yaml
```
...
```bash
openclaw tyk-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw tyk-guide | openclaw task-decomposer
```
