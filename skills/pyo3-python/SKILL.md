...
```bash
npx clawhub@latest install pyo3-python
```
...
```bash
# 使用默认配置
openclaw pyo3-python

# 查看帮助
openclaw pyo3-python --help

# 指定配置
openclaw pyo3-python --config ./config.yaml
```
...
```bash
openclaw pyo3-python --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw pyo3-python | openclaw task-decomposer
```
