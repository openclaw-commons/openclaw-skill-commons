...
```bash
npx clawhub@latest install mesos-guide
```
...
```bash
# 使用默认配置
openclaw mesos-guide

# 查看帮助
openclaw mesos-guide --help

# 指定配置
openclaw mesos-guide --config ./config.yaml
```
...
```bash
openclaw mesos-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw mesos-guide | openclaw task-decomposer
```
