...
```bash
npx clawhub@latest install nginx-config
```
...
```bash
# 使用默认配置
openclaw nginx-config

# 查看帮助
openclaw nginx-config --help

# 指定配置
openclaw nginx-config --config ./config.yaml
```
...
```bash
openclaw nginx-config --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw nginx-config | openclaw task-decomposer
```
