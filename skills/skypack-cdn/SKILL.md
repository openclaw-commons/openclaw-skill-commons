...
```bash
npx clawhub@latest install skypack-cdn
```
...
```bash
# 使用默认配置
openclaw skypack-cdn

# 查看帮助
openclaw skypack-cdn --help

# 指定配置
openclaw skypack-cdn --config ./config.yaml
```
...
```bash
openclaw skypack-cdn --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw skypack-cdn | openclaw task-decomposer
```
