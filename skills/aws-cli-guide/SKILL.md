...
```bash
npx clawhub@latest install aws-cli-guide
```
...
```bash
# 使用默认配置
openclaw aws-cli-guide

# 查看帮助
openclaw aws-cli-guide --help

# 指定配置
openclaw aws-cli-guide --config ./config.yaml
```
...
```bash
openclaw aws-cli-guide --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw aws-cli-guide | openclaw task-decomposer
```
