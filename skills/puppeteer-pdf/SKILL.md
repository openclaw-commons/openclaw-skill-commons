...
```bash
npx clawhub@latest install puppeteer-pdf
```
...
```bash
# 使用默认配置
openclaw puppeteer-pdf

# 查看帮助
openclaw puppeteer-pdf --help

# 指定配置
openclaw puppeteer-pdf --config ./config.yaml
```
...
```bash
openclaw puppeteer-pdf --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw puppeteer-pdf | openclaw task-decomposer
```
