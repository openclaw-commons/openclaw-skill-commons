...
```bash
npx clawhub@latest install koa-async
```
...
# 查看帮助
openclaw koa-async --help

# 指定配置
openclaw koa-async --config ./config.yaml
```
...
```bash
openclaw koa-async --verbose --output ./output
```

### 与其他技能组合

```bash
openclaw koa-async | openclaw task-decomposer
```
